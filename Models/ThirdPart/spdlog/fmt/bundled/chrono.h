...
To safe_duration_cast(std::chrono::duration<FromRep, FromPeriod> from, int &ec)
{
    ...
    using IntermediateRep = typename std::
        common_type<typename From::rep, typename To::rep, decltype(Factor::num)>::type;
...
template <typename CodeUnit>
void write_codecvt(
    codecvt_result<CodeUnit> &out, string_view in_buf, const std::locale &loc)
{
...
template <
    typename Char, typename OutputIt, FMT_ENABLE_IF(!std::is_same<Char, char>::value)>
...
inline void do_write(
    buffer<Char> &buf, const std::tm &time, const std::locale &loc, char format,
    char modifier)
{
...
template <
    typename Char, typename OutputIt, FMT_ENABLE_IF(!std::is_same<Char, char>::value)>
auto write(
    OutputIt out, const std::tm &time, const std::locale &loc, char format,
...
write_digit2_separated(char *buf, unsigned a, unsigned b, unsigned c, char sep)
{
    ...
    digits =
        ((digits & 0x00f00000f00000f0) >> 4) | ((digits & 0x000f00000f00000f) << 8);
...
parse_chrono_format(const Char *begin, const Char *end, Handler &&handler)
{
    ...
    while (ptr != end)
    {
        ...
        // Hour, minute, second:
        case 'H': handler.on_24_hour(numeric_system::standard); break;
        case 'I': handler.on_12_hour(numeric_system::standard); break;
        case 'M': handler.on_minute(numeric_system::standard); break;
        case 'S': handler.on_second(numeric_system::standard); break;
        // Other:
        ...
        case 'z': handler.on_utc_offset(); break;
        ...
        case 'O':
            ...
            case 'H': handler.on_24_hour(numeric_system::alternative); break;
            case 'I': handler.on_12_hour(numeric_system::alternative); break;
            case 'M': handler.on_minute(numeric_system::alternative); break;
            case 'S': handler.on_second(numeric_system::alternative); break;
            default: FMT_THROW(format_error("invalid format"));
            }
            break;
        default: FMT_THROW(format_error("invalid format"));
...
template <
    long long Num, long long Den, int N = 0,
...
template <
    typename Char, typename Rep, typename OutputIt,
...
struct chrono_formatter
{
    ...
    chrono_formatter(
        FormatContext &ctx, OutputIt o, std::chrono::duration<Rep, Period> d) :
        context(ctx),
        out(o), val(static_cast<rep>(d.count())), negative(false)
    {
        if (d.count() < 0)
        {
            val      = 0 - val;
    ...
    void write_fractional_seconds(Duration d)
    {
        ...
        using subsecond_precision = std::chrono::duration<
            typename std::
                common_type<typename Duration::rep, std::chrono::seconds::rep>::type,
        ...
        if (ns == numeric_system::standard)
        {
            if (std::is_floating_point<rep>::value)
            {
                ...
                format_to(
                    ...
                    runtime("{:.{}f}"),
                    std::fmod(
                        val * static_cast<rep>(Period::num)
