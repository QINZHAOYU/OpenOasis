...
// written to it.
template <
    typename OutputIt, typename Enable = typename std::is_void<
...
get([[maybe_unused]] const T &first, [[maybe_unused]] const Args &...rest)
{
...
template <
    typename T, typename Args, size_t END_POS, int ARG_INDEX, int NEXT_ID, typename S>
...
constexpr auto compile_format_string(S format_str)
{
    ...
    else
    {
        ...
        {
            return parse_tail<Args, end, ID>(
                make_text(str, POS, end - POS), format_str);
        }
        else
        {
            return parse_tail<Args, end, ID>(
                code_unit<char_type>{str[POS]}, format_str);
        }
    }
}

template <
    typename... Args, typename S, FMT_ENABLE_IF(detail::is_compiled_string<S>::value)>
...
template <
    ...
    FMT_ENABLE_IF(detail::is_compiled_format<CompiledFormat>::value)>
constexpr FMT_INLINE OutputIt
format_to(OutputIt out, const CompiledFormat &cf, const Args &...args)
{
    return cf.format(out, args...);
}

template <
    typename S, typename... Args, FMT_ENABLE_IF(detail::is_compiled_string<S>::value)>
FMT_INLINE std::basic_string<typename S::char_type> format(const S &, Args &&...args)
{
    ...
    if constexpr (std::is_same<
                      remove_cvref_t<decltype(compiled)>,
...
format_to_n(OutputIt out, size_t n, const S &format_str, Args &&...args)
{
    auto it = fmt::format_to(
        detail::truncating_iterator<OutputIt>(out, n),
        format_str,
        std::forward<Args>(args)...);
