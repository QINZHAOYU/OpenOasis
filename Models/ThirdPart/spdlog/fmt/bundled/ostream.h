...
private:
    template <typename U>
    static auto test(int) -> bool_constant<
        sizeof(std::declval<std::basic_ostream<Char> &>() << std::declval<U>()) != 0>;
...
#elif defined(_WIN32) && defined(_LIBCPP_VERSION)
template class file_access<
    file_access_tag, std::__stdoutbuf<char>, &std::__stdoutbuf<char>::__file_>;
...
struct formatter<detail::streamed_view<T>, Char> : basic_ostream_formatter<Char>
{
    ...
    auto format(
        detail::streamed_view<T> view, basic_format_context<OutputIt, Char> &ctx) const
        -> OutputIt
    {
...
void vprint(
    std::basic_ostream<Char> &os, basic_string_view<type_identity_t<Char>> format_str,
...
void print(
    std::wostream &os, basic_format_string<wchar_t, type_identity_t<Args>...> fmt,
    Args &&...args)
{
