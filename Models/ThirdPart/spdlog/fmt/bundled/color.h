...
void vprint(
    std::FILE *f, const text_style &ts, const S &format,
    basic_format_args<buffer_context<type_identity_t<Char>>> args)
{
...
{
    vprint(
        f, ts, format_str, fmt::make_format_args<buffer_context<char_t<S>>>(args...));
}
...
inline auto
format_to(OutputIt out, const text_style &ts, const S &format_str, Args &&...args) ->
