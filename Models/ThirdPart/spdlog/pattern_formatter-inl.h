...
public:
    scoped_padder(
        size_t wrapped_size, const padding_info &padinfo, memory_buf_t &dest) :
        padinfo_(padinfo),
        dest_(dest)
    {
        remaining_pad_ =
...
{
    null_scoped_padder(
        size_t /*wrapped_size*/, const padding_info & /*padinfo*/,
        memory_buf_t & /*dest*/)
    {}
...
public:
    ...
    void format(
        const details::log_msg &, const std::tm &tm_time, memory_buf_t &dest) override
    {
...
public:
    ...
    void format(const details::log_msg &msg, const std::tm &tm_time, memory_buf_t &dest)
        override
    {
        const size_t field_size = 6;
        ScopedPadder p(field_size, padinfo_, dest);
    ...
    static const char *basename(const char *filename)
    {
        ...
        else
        {
            ...
            const auto it = std::find_first_of(
                begin, end, std::begin(os::folder_seps), std::end(os::folder_seps) - 1);
...
SPDLOG_INLINE pattern_formatter::pattern_formatter(
    ...
    eol_(std::move(eol)), pattern_time_type_(time_type), need_localtime_(false),
...
pattern_formatter::pattern_formatter(pattern_time_type time_type, std::string eol) :
    pattern_("%+"),
    eol_(std::move(eol)), pattern_time_type_(time_type), need_localtime_(true),
    last_log_secs_(0)
{
    std::memset(&cached_tm_, 0, sizeof(cached_tm_));
...
SPDLOG_INLINE void
pattern_formatter::format(const details::log_msg &msg, memory_buf_t &dest)
{
    if (need_localtime_)
    {
...
pattern_formatter::handle_flag_(char flag, details::padding_info padding)
{
    ...
    case ('u'):  // elapsed time since last log message in nanos
        formatters_.push_back(
            details::make_unique<
                details::elapsed_formatter<Padder, std::chrono::nanoseconds>>(padding));
        break;
    ...
    default:  // Unknown flag appears as is
        ...
        // fix issue #1617 (prev char was '!' and should have been treated as funcname
        // flag instead of truncating flag) spdlog::set_pattern("[%10!] %v") => "[ main]
        // some message" spdlog::set_pattern("[%3!!] %v") => "[mai] some message"
        else
        {
