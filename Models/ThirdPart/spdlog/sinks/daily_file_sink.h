...
#pragma GCC diagnostic ignored "-Wformat-nonliteral"
    ...
    static size_t
    strftime(char *str, size_t count, const char *format, const std::tm *time)
    {
        return std::strftime(str, count, format, time);
    }

    static size_t
    strftime(wchar_t *str, size_t count, const wchar_t *format, const std::tm *time)
    {
...
public:
    ...
    daily_file_sink(
        filename_t base_filename, int rotation_hour, int rotation_minute,
    ...
    void delete_old_()
    {
        ...
        if (filenames_q_.full())
        {
            ...
            if (!ok)
            {
                ...
                throw_spdlog_ex(
                    "Failed removing daily file " + filename_to_str(old_filename),
                    errno);
            }
        }
        filenames_q_.push_back(std::move(current_file));
