...
template <typename ConsoleMutex>
void SPDLOG_INLINE
wincolor_sink<ConsoleMutex>::set_color(level::level_enum level, std::uint16_t color)
{
...
wincolor_sink<ConsoleMutex>::set_foreground_color_(std::uint16_t attribs)
{
    ...
    if (!::GetConsoleScreenBufferInfo(
            static_cast<HANDLE>(out_handle_), &orig_buffer_info))
    {
    ...
    auto ignored = ::SetConsoleTextAttribute(
        static_cast<HANDLE>(out_handle_), static_cast<WORD>(new_attribs));
    (void)(ignored);
    ...
    const memory_buf_t &formatted, size_t start, size_t end)
{
    if (end > start)
    {
        ...
        auto ignored = ::WriteConsoleA(
            static_cast<HANDLE>(out_handle_),
