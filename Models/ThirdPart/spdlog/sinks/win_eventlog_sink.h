...
// Writing to Windows Event Log requires the registry entries below to be present, with
// the following modifications:
// 1. <log_name>    should be replaced with your log name (e.g. your application name)
// 2. <source_name> should be replaced with the specific source name and the key should
// be duplicated for
//                  each source used in the application
//
// Since typically modifications of this kind require elevation, it's better to do it as
// a part of setup procedure. The snippet below uses mscoree.dll as the message file as
// it exists on most of the Windows systems anyway and happens to contain the needed
// resource.
//
// You can also specify a custom message file if needed.
// Please refer to Event Log functions descriptions in MSDN for more details on custom
// message files.
...
struct win32_error : public spdlog_ex
{
    ...
    static std::string
    format(std::string const &user_message, DWORD error_code = GetLastError())
    {
...
public:
    ...
    static sid_t get_current_user_sid()
    {
        ...
        } current_process_token(::GetCurrentProcess());  // GetCurrentProcess returns
                                                         // pseudohandle, no leak here!

        // Get the required size, this is expected to fail with
        // ERROR_INSUFFICIENT_BUFFER and return the token size
        DWORD tusize = 0;
        if (::GetTokenInformation(
                current_process_token.token_handle_, TokenUser, NULL, 0, &tusize))
        {
