...
public:
    ...
    try : mongo_sink(
        std::make_shared<mongocxx::instance>(), db_name, collection_name, uri)
    {}
    ...
    mongo_sink(
        std::shared_ptr<mongocxx::instance> instance, const std::string &db_name,
        const std::string &collection_name,
        const std::string &uri = "mongodb://localhost:27017") :
        instance_(std::move(instance)),
        db_name_(db_name), coll_name_(collection_name)
    {
        try
        {
            client_ =
