...
enum WriteFlag
{
    ...
    kWriteDefaultFlags =
        CEREAL_RAPIDJSON_WRITE_DEFAULT_FLAGS  //!< Default write flags. Can be
...
*/
template <
    typename OutputStream, typename SourceEncoding = UTF8<>,
...
public:
    ...
    /*! \param os Output stream.
        \param stackAllocator User supplied allocator. If it is null, it will create a
    ...
    */
    ...
    /*!
       ...
       truncate significand for positive exponent) writer.Double(1.23e-4); // "0.0" (do
       truncate significand for negative exponent) writer.EndArray(); \endcode

        The default setting does not truncate any decimal places. You can restore to
       this setting by calling \code
    ...
    */
    ...
    /*!
       ...
       within [0, length - 1] range. \param length Length of the json. \param type Type
       of the root of json.
    */
    ...
    void Prefix(Type type)
    {
        ...
        {  // this value is not at root
            ...
            if (!level->inArray && level->valueCount % 2 == 0)
                CEREAL_RAPIDJSON_ASSERT(
                    type == kStringType);  // if it's in object, then even number should
                                           // be a name
            level->valueCount++;
        }
        else
...
template <>
inline bool
Writer<StringBuffer>::ScanWriteUnescapedString(StringStream &is, size_t length)
{
    if (length < 16)
        return CEREAL_RAPIDJSON_LIKELY(is.Tell() < length);
