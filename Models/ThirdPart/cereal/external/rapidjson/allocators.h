...
/*! This allocator allocate memory blocks from pre-allocated memory chunks.
    ...
    \tparam BaseAllocator the allocator type for allocating memory chunks. Default is
   CrtAllocator. \note implements Allocator concept
*/
...
public:
    static const bool kNeedFree = false;  //!< Tell users that no need to call Free()
                                          //!< with this allocator. (concept Allocator)
    ...
    */
    MemoryPoolAllocator(
        ...
        chunkHead_(0),
        chunk_capacity_(chunkSize), userBuffer_(0), baseAllocator_(baseAllocator),
        ownBaseAllocator_(0)
    {}

    //! Constructor with user-supplied buffer.
    /*! The user buffer will be used firstly. When it is full, memory pool allocates new
       ...
       sizeof(ChunkHeader). \param chunkSize The size of memory chunk. The default is
       kDefaultChunkSize. \param baseAllocator The allocator for allocating memory
       chunks.
    */
    MemoryPoolAllocator(
        void *buffer, size_t size, size_t chunkSize = kDefaultChunkCapacity,
        BaseAllocator *baseAllocator = 0) :
        chunkHead_(0),
        chunk_capacity_(chunkSize), userBuffer_(buffer), baseAllocator_(baseAllocator),
        ownBaseAllocator_(0)
    {
    ...
    void *Malloc(size_t size)
    {
        ...
        void *buffer = reinterpret_cast<char *>(chunkHead_)
                       + CEREAL_RAPIDJSON_ALIGN(sizeof(ChunkHeader)) + chunkHead_->size;
...
private:
    ...
    */
    ...
    static const int kDefaultChunkCapacity =
        CEREAL_RAPIDJSON_ALLOCATOR_DEFAULT_CHUNK_CAPACITY;  //!< Default chunk capacity.
    ...
    ChunkHeader *chunkHead_;  //!< Head of the chunk linked-list. Only the head chunk
                              //!< serves allocation.
