...
unicode's definition.
    ...
    //! \note This function just validating and copying the codepoint without actually
decode it. template <typename InputStream, typename OutputStream> static bool
...
#undef CEREAL_RAPIDJSON_TAIL
    ...
    static unsigned char GetRange(unsigned char c)
    {
        ...
        // With new mapping 1 -> 0x10, 7 -> 0x20, 9 -> 0x40, such that AND operation can
        // test multiple types.
...
/*! http://en.wikipedia.org/wiki/UTF-16
    ...
    \tparam CharType Type for storing 16-bit UTF-16 data. Default is wchar_t. C++11 may
   use char16_t instead. \note implements Encoding concept

    \note For in-memory access, no need to concern endianness. The code units and code
   points are represented by CPU's endianness. For streaming, use UTF16LE and UTF16BE,
   which handle endianness.
*/
...
/*! http://en.wikipedia.org/wiki/UTF-32
    \tparam CharType Type for storing 32-bit UTF-32 data. Default is unsigned. C++11 may
   use char32_t instead. \note implements Encoding concept

    \note For in-memory access, no need to concern endianness. The code units and code
   points are represented by CPU's endianness. For streaming, use UTF32LE and UTF32BE,
   which handle endianness.
*/
...
//! Dynamically select encoding according to stream's runtime-specified UTF encoding
//! type.
/*! \note This class can be used with AutoUTFInputtStream and AutoUTFOutputStream, which
 * provides GetType().
 */
...
#define CEREAL_RAPIDJSON_ENCODINGS_FUNC(x)                                             \
    ...
    template <typename InputStream>
    static CEREAL_RAPIDJSON_FORCEINLINE bool
...
struct Transcoder
{
    //! Take one Unicode codepoint from source encoding, convert it to target encoding
