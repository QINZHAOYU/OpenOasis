...
/*!
    ...
    \note This is a Thompson NFA engine, implemented with reference to
        Cox, Russ. "Regular Expression Matching Can Be Simple And Fast (but is slow in
   Java, Perl, PHP, Python, Ruby,...).", https://swtch.com/~rsc/regexp/regexp1.html
*/
...
public:
    ...
    GenericRegex(const Ch *source, Allocator *allocator = 0) :
        ...
        ranges_(allocator_, 256), root_(kRegexInvalidState), stateCount_(),
        rangeCount_(), anchorBegin_(), anchorEnd_()
    {
...
private:
    ...
    void Parse(DecodedStream<InputStream, Encoding> &ds)
    {
        ...
        Stack<Allocator> atomCountStack(
            allocator_, 256);  // unsigned (Atom per parenthesis)
        ...
        while (ds.Peek() != 0)
        {
            ...
            case '[': {
                ...
                if (!ParseRange(ds, &range))
                    return;
                SizeType s = NewState(
                    kRegexInvalidState, kRegexInvalidState, kRangeCharacterClass);
    ...
    bool CharacterEscape(
        DecodedStream<InputStream, Encoding> &ds, unsigned *escapedCodepoint)
    {
        unsigned codepoint;
        switch (codepoint = ds.Take())
        {
