...
public:
    ...
    // Expansion is run very infrequently, so it is moved to another (probably
    // non-inline) function.
    template <typename T>
    CEREAL_RAPIDJSON_FORCEINLINE void Reserve(size_t count = 1)
    {
        ...
        if (CEREAL_RAPIDJSON_UNLIKELY(
                static_cast<std::ptrdiff_t>(sizeof(T) * count)
                > (stackEnd_ - stackTop_)))
            Expand<T>(count);
    }
...
private:
    ...
    {
        // Only expand the capacity if the current stack exists. Otherwise just create a
