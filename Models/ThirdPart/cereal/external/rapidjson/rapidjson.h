...
#define CEREAL_RAPIDJSON_VERSION_STRING                                                \
    CEREAL_RAPIDJSON_STRINGIFY(                                                        \
        CEREAL_RAPIDJSON_MAJOR_VERSION.CEREAL_RAPIDJSON_MINOR_VERSION                  \
...
/*! \def CEREAL_RAPIDJSON_HAS_STDSTRING
    ...
    By defining this preprocessor symbol to \c 1, several convenience functions for
   using \ref rapidjson::GenericValue with \c std::string are enabled, especially for
...
/*!
    ...
    This optimization uses the fact that current X86-64 architecture only implement
   lower 48-bit virtual address. The higher 16-bit can be used for storing other data.
    \c GenericValue uses this optimization to reduce its size form 24 bytes to 16 bytes
    ...
    (p = reinterpret_cast<type *>(                                                     \
         (reinterpret_cast<uintptr_t>(p)                                               \
          & static_cast<uintptr_t>(                                                    \
              CEREAL_RAPIDJSON_UINT64_C2(0xFFFF0000, 0x00000000)))                     \
...
#define CEREAL_RAPIDJSON_STATIC_ASSERT(x)                                              \
    ...
    CEREAL_RAPIDJSON_JOIN(StaticAssertTypedef, __LINE__)                               \
        CEREAL_RAPIDJSON_STATIC_ASSERT_UNUSED_ATTRIBUTE
#endif  // CEREAL_RAPIDJSON_STATIC_ASSERT
...
#define CEREAL_RAPIDJSON_GNUC                                                          \
    ...
    || (defined(CEREAL_RAPIDJSON_GNUC)                                                 \
        && CEREAL_RAPIDJSON_GNUC >= CEREAL_RAPIDJSON_VERSION_CODE(4, 2, 0))
...
#define CEREAL_RAPIDJSON_DIAG_OFF(x)                                                   \
    CEREAL_RAPIDJSON_DIAG_PRAGMA(                                                      \
        ignored CEREAL_RAPIDJSON_STRINGIFY(CEREAL_RAPIDJSON_JOIN(-W, x)))

// push/pop support in Clang and GCC>=4.6
#if defined(__clang__)                                                                 \
    || (defined(CEREAL_RAPIDJSON_GNUC)                                                 \
        && CEREAL_RAPIDJSON_GNUC >= CEREAL_RAPIDJSON_VERSION_CODE(4, 6, 0))
...
#elif (                                                                                \
    defined(CEREAL_RAPIDJSON_GNUC)                                                     \
    && (CEREAL_RAPIDJSON_GNUC >= CEREAL_RAPIDJSON_VERSION_CODE(4, 3, 0))               \
