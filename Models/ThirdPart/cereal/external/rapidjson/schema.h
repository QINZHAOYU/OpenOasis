...
#define CEREAL_RAPIDJSON_SCHEMA_USE_INTERNALREGEX 0
    ...
    && defined(CEREAL_RAPIDJSON_SCHEMA_USE_STDREGEX)                                   \
...
public:
    ...
    virtual void
    AboveMaximum(int64_t actual, const SValue &expected, bool exclusive) = 0;
    virtual void
    AboveMaximum(uint64_t actual, const SValue &expected, bool exclusive) = 0;
    virtual void
    AboveMaximum(double actual, const SValue &expected, bool exclusive) = 0;
    virtual void
    BelowMinimum(int64_t actual, const SValue &expected, bool exclusive) = 0;
    virtual void
    BelowMinimum(uint64_t actual, const SValue &expected, bool exclusive) = 0;
    virtual void
    BelowMinimum(double actual, const SValue &expected, bool exclusive) = 0;
    ...
    virtual void
    PropertyViolations(ISchemaValidator **subvalidators, SizeType count) = 0;
...
private:
    ...
    static uint64_t Hash(uint64_t h, uint64_t d)
    {
        static const uint64_t kPrime =
            CEREAL_RAPIDJSON_UINT64_C2(0x00000100, 0x000001b3);
        h ^= d;
        h *= kPrime;
        return h;
    }
...
struct SchemaValidationContext
{
    ...
    SchemaValidationContext(
        SchemaValidatorFactoryType &f, ErrorHandlerType &eh, const SchemaType *s) :
        factory(f),
        error_handler(eh), schema(s), valueSchema(), invalidKeyword(), hasher(),
        arrayElementHashCodes(), validators(), validatorCount(),
        patternPropertiesValidators(), patternPropertiesValidatorCount(),
        patternPropertiesSchemas(), patternPropertiesSchemaCount(),
...
public:
    ...
    Schema(
        SchemaDocumentType *schemaDocument, const PointerType &p,
        const ValueType &value, const ValueType &document, AllocatorType *allocator) :
        allocator_(allocator),
        uri_(schemaDocument->GetURI(), *allocator), pointer_(p, allocator),
        typeless_(schemaDocument->GetTypeless()), enum_(), enumCount_(), not_(),
        ...
        propertyCount_(), minProperties_(), maxProperties_(SizeType(~0)),
        additionalProperties_(true), hasDependencies_(), hasRequired_(),
        hasSchemaDependencies_(), additionalItemsSchema_(), itemsList_(), itemsTuple_(),
                    ...
                    if (itr->value.IsArray())
                    {
                             ...
                             ++targetItr)
                        {
                            ...
                            if (FindPropertyIndex(*targetItr, &targetIndex))
                                properties_[sourceIndex].dependencies[targetIndex] =
                                    true;
                        }
                    }
    ...
    bool StartObject(Context &context) const
    {
        ...
        {  // pre-allocate schema array
            ...
            std::memset(
                context.patternPropertiesSchemas, 0, sizeof(SchemaType *) * count);
        }
...
#define CEREAL_RAPIDJSON_STRING_(name, ...)                                            \
    ...
    CEREAL_RAPIDJSON_STRING_(
        AdditionalItems, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'I', 't',
        'e', 'm', 's')
    CEREAL_RAPIDJSON_STRING_(
    ...
    CEREAL_RAPIDJSON_STRING_(
        ExclusiveMinimum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'i', 'n',
        'i', 'm', 'u', 'm')
    CEREAL_RAPIDJSON_STRING_(
        ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'a', 'x',
        'i', 'm', 'u', 'm')
    CEREAL_RAPIDJSON_STRING_(
...
private:
    ...
    void AssignIfExist(
        SchemaArray &out, SchemaDocumentType &schemaDocument, const PointerType &p,
    ...
    bool FindPropertyIndex(const ValueType &name, SizeType *outIndex) const
    {
        ...
        for (SizeType index = 0; index < propertyCount_; index++)
            if (properties_[index].name.GetStringLength() == len
                && (std::memcmp(
                        properties_[index].name.GetString(), str, sizeof(Ch) * len)
                    == 0))
            {
                *outIndex = index;
                return true;
...
struct TokenHelper
{
    CEREAL_RAPIDJSON_FORCEINLINE static void
    AppendIndexToken(Stack &documentStack, SizeType index)
    {
        *documentStack.template Push<Ch>() = '/';
        char   buffer[21];
        size_t length = static_cast<size_t>(
            (sizeof(SizeType) == 4 ? u32toa(index, buffer) : u64toa(index, buffer))
            - buffer);
...
public:
    ...
    GetRemoteDocument(const Ch *uri, SizeType length) = 0;
};
...
/*!
   ...
   determine the encoding. \tparam Allocator Allocator type for allocating memory of
   this document.
*/
...
public:
    ...
    /*!
       ...
       remoteProvider An optional remote schema document provider for resolving remote
       reference. Can be null. \param allocator An optional allocator instance for
       allocating memory. Can be null.
    */
    explicit GenericSchemaDocument(
        ...
        allocator_(allocator), ownAllocator_(), root_(), typeless_(),
...
private:
    ...
    {
        SchemaRefEntry(
            const PointerType &s, const PointerType &t, const SchemaType **outSchema,
            Allocator *allocator) :
            source(s, allocator),
            target(t, allocator), schema(outSchema)
        {}
        PointerType        source;
        PointerType        target;
        const SchemaType **schema;
    ...
    void CreateSchemaRecursive(
        const SchemaType **schema, const PointerType &pointer, const ValueType &v,
        ...
        if (v.GetType() == kObjectType)
        {
            ...
            for (typename ValueType::ConstMemberIterator itr = v.MemberBegin();
                ...
                CreateSchemaRecursive(
                    0, pointer.Append(itr->name, allocator_), itr->value, document);
        }
    ...
    bool HandleRefSchema(
        const PointerType &source, const SchemaType **schema, const ValueType &v,
        const ValueType &document)
    {
...
public:
    ...
    */
    GenericSchemaValidator(
        ...
        root_(schemaDocument.GetRoot()), stateAllocator_(allocator),
    ...
    */
    ...
    {
        AddNumberError(
            SchemaType::GetMultipleOfString(), ValueType(actual).Move(), expected);
    }
...
#define CEREAL_RAPIDJSON_STRING_(name, ...)                                            \
    ...
    CEREAL_RAPIDJSON_STRING_(
        InstanceRef, 'i', 'n', 's', 't', 'a', 'n', 'c', 'e', 'R', 'e', 'f')
    ...
    CEREAL_RAPIDJSON_STRING_(
        Disallowed, 'd', 'i', 's', 'a', 'l', 'l', 'o', 'w', 'e', 'd')
...
#define CEREAL_RAPIDJSON_SCHEMA_HANDLE_VALUE_(method, arg1, arg2)                      \
    ...
    {
        CEREAL_RAPIDJSON_SCHEMA_HANDLE_VALUE_(
...
private:
    ...
    GenericSchemaValidator(
        ...
        size_t          documentStackCapacity = kDefaultDocumentStackCapacity) :
        schemaDocument_(&schemaDocument),
        root_(root), stateAllocator_(allocator), ownStateAllocator_(0),
...
#endif
    {
        if (basePath && basePathSize)
            memcpy(
                documentStack_.template Push<char>(basePathSize),
                basePath,
                basePathSize);
    }
    ...
    StateAllocator &GetStateAllocator()
    {
        if (!stateAllocator_)
            stateAllocator_ = ownStateAllocator_ =
                CEREAL_RAPIDJSON_NEW(StateAllocator)();
    ...
    void AddErrorLocation(ValueType &result, bool parent)
    {
        ...
        ((parent && instancePointer.GetTokenCount() > 0) ? PointerType(
             instancePointer.GetTokens(), instancePointer.GetTokenCount() - 1) :
                                                           instancePointer)
    ...
    void
    AddCurrentError(const typename SchemaType::ValueType &keyword, bool parent = false)
    {
    ...
    void AddErrorArray(
        const typename SchemaType::ValueType &keyword, ISchemaValidator **subvalidators,
        SizeType count)
    {
        ValueType errors(kArrayType);
        for (SizeType i = 0; i < count; ++i)
    ...
    internal::Stack<StateAllocator>
        documentStack_;  //!< stack to store the current path of validating document
                         //!< (Ch)
...
*/
template <
    unsigned parseFlags, typename InputStream, typename SourceEncoding,
