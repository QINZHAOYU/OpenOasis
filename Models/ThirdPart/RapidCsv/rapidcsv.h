...
struct ConverterParams
{
    /**
     * @brief   Constructor
     * @param   pHasDefaultConverter  specifies if conversion of non-numerical strings
     * shall be converted to a default numerical value, instead of causing an exception
     * to be thrown (default).
     * @param   pDefaultFloat         floating-point default value to represent invalid
     * numbers.
    ...
    /**
     * @brief   specifies if conversion of non-numerical strings shall be converted to a
     * default numerical value, instead of causing an exception to be thrown (default).
     */
...
/**
 * @brief     Exception thrown when attempting to access Document data in a datatype
 * which is not supported by the Converter class.
 */
...
public:
    /**
     * @brief   Constructor
     * @param   pConverterParams      specifies how conversion of non-numerical values
     * to numerical datatype shall be handled.
     */
...
template <>
inline void
Converter<std::string>::ToStr(const std::string &pVal, std::string &pStr) const
{
    pStr = pVal;
}
...
/**
 * @brief     Datastructure holding parameters controlling which row and column should
...
struct LabelParams
{
    /**
     * @brief   Constructor
     * @param   pColumnNameIdx        specifies the zero-based row index of the column
     * labels, setting it to -1 prevents column lookup by label name, and gives access
     *                                to all rows as document data. Default: 0
     * @param   pRowNameIdx           specifies the zero-based column index of the row
     * labels, setting it to -1 prevents row lookup by label name, and gives access to
     * all columns as document data. Default: -1
     */
    explicit LabelParams(
        const ssize_t pColumnNameIdx = 0, const ssize_t pRowNameIdx = -1) :
...
/**
 * @brief     Datastructure holding parameters controlling how the CSV data fields are
 * separated.
 */
struct SeparatorParams
{
     ...
     * @param   pTrim                 specifies whether to trim leading and trailing
     * spaces from cells read (default false).
     * @param   pHasCR                specifies whether a new document (i.e. not an
     * existing document read) should use CR/LF instead of only LF (default is to use
     * standard behavior of underlying platforms - CR/LF for Win, and LF for others).
     * @param   pQuotedLinebreaks     specifies whether to allow line breaks in quoted
     * text (default false)
     * @param   pAutoQuote            specifies whether to automatically dequote data
     * during read, and add quotes during write (default true).
    ...
    explicit SeparatorParams(
        ...
        mTrim(pTrim), mHasCR(pHasCR), mQuotedLinebreaks(pQuotedLinebreaks),
...
/**
 * @brief     Datastructure holding parameters controlling how special line formats
 * should be treated.
 */
struct LineReaderParams
{
     ...
     * @param   pCommentPrefix        specifies which prefix character to indicate a
     * comment line. Default: #
     * @param   pSkipEmptyLines       specifies whether to skip empty lines. Default:
     * false
     */
    explicit LineReaderParams(
        ...
        mCommentPrefix(pCommentPrefix), mSkipEmptyLines(pSkipEmptyLines)
    {}
...
public:
     ...
     * @param   pLabelParams          specifies which row and column should be treated
     * as labels.
     * @param   pSeparatorParams      specifies which field and row separators should be
     * used.
     * @param   pConverterParams      specifies how invalid numbers (including empty
    ...
    explicit Document(
        ...
        mPath(pPath),
        mLabelParams(pLabelParams), mSeparatorParams(pSeparatorParams),
        mConverterParams(pConverterParams), mLineReaderParams(pLineReaderParams),
        mData(), mColumnNames(), mRowNames()
    {
        if (!mPath.empty())
        {
            ReadCsv();
    ...
    /**
     * @brief   Constructor
     * @param   pStream               specifies a binary input stream to read CSV data
     * from.
     * @param   pLabelParams          specifies which row and column should be treated
     * as labels.
     * @param   pSeparatorParams      specifies which field and row separators should be
     ...
     */
    void Load(
        const std::string &pPath, const LabelParams &pLabelParams = LabelParams(),
        const SeparatorParams  &pSeparatorParams  = SeparatorParams(),
        const ConverterParams  &pConverterParams  = ConverterParams(),
        const LineReaderParams &pLineReaderParams = LineReaderParams())
    {
     ...
     * @param   pPath                 optionally specifies the path where the CSV-file
     * will be created (if not specified, the original path provided when creating or
     ...
     * @param   pStream               specifies a binary output stream to write the data
     * to.
     */
    ...
    ssize_t GetColumnIdx(const std::string &pColumnName) const
    {
        if (mLabelParams.mColumnNameIdx >= 0)
        {
            ...
            {
                return static_cast<ssize_t>(mColumnNames.at(pColumnName))
        ...
        for (auto itRow = pColumn.begin(); itRow != pColumn.end(); ++itRow)
        {
            ...
            mData
                .at(static_cast<size_t>(
                    std::distance(pColumn.begin(), itRow) + mLabelParams.mColumnNameIdx
                    + 1))
    ...
    template <typename T>
    void InsertColumn(
        const size_t pColumnIdx, const std::vector<T> &pColumn = std::vector<T>(),
        const std::string &pColumnName = std::string())
    {
        ...
        for (auto itRow = mData.begin(); itRow != mData.end(); ++itRow)
        {
            if (std::distance(mData.begin(), itRow) >= mLabelParams.mColumnNameIdx)
            {
                ...
                itRow->insert(
                    itRow->begin() + static_cast<ssize_t>(dataColumnIdx),
                    column.at(rowIdx));
            }
        }
    ...
    ssize_t GetRowIdx(const std::string &pRowName) const
    {
        if (mLabelParams.mRowNameIdx >= 0)
        {
            ...
            {
                return static_cast<ssize_t>(mRowNames.at(pRowName))
                       - (mLabelParams.mColumnNameIdx + 1);
            }
        }
        return -1;
    ...
    void SetRow(const size_t pRowIdx, const std::vector<T> &pRow)
    {
        ...
        for (auto itCol = pRow.begin(); itCol != pRow.end(); ++itCol)
        {
            ...
            mData.at(dataRowIdx)
                .at(static_cast<size_t>(
                    std::distance(pRow.begin(), itCol) + mLabelParams.mRowNameIdx
                    + 1)) = str;
        }
    }
    ...
    template <typename T>
    T GetCell(
        const std::string &pColumnName, const size_t pRowIdx, ConvFunc<T> pToVal) const
    {
        const ssize_t columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0)
        {
    ...
    template <typename T>
    T GetCell(
        const size_t pColumnIdx, const std::string &pRowName, ConvFunc<T> pToVal) const
    {
        const ssize_t rowIdx = GetRowIdx(pRowName);
        if (rowIdx < 0)
        {
    ...
    template <typename T>
    void
    SetCell(const std::string &pColumnName, const std::string &pRowName, const T &pCell)
    {
        const ssize_t columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0)
        {
    ...
    void SetColumnName(size_t pColumnIdx, const std::string &pColumnName)
    {
        ...
        mData.at(static_cast<size_t>(mLabelParams.mColumnNameIdx)).at(dataColumnIdx) =
            pColumnName;
    }
...
private:
    ...
    void ParseCsv(std::istream &pStream, std::streamsize p_FileLength)
    {
        ...
        while (p_FileLength > 0)
        {
            ...
            // With user-specified istream opened in non-binary mode on windows, we may
            // have a data length mismatch, so ensure we don't parse outside actual data
            // length read.
    ...
    void WriteCsv() const
    {
        ...
        if (mIsUtf16)
        {
            ...
            {
                wstream.imbue(std::locale(
                    wstream.getloc(), new std::codecvt_utf16<wchar_t, 0x10ffff>));
            }
    ...
    std::string Trim(const std::string &pStr) const
    {
        if (mSeparatorParams.mTrim)
        {
            ...
            str.erase(
                std::find_if(
                    str.rbegin(), str.rend(), [](int ch) { return !isspace(ch); })
                    .base(),
                str.end());
...
#pragma warning(pop)
    ...
    static void ReplaceString(
        std::string &pStr, const std::string &pSearch, const std::string &pReplace)
    {
        size_t pos = 0;
