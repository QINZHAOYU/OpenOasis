...
//
// The algorithm below is a reimplementation of former \src\LU\Inverse_SSE.h using
// PacketMath. inv(M) = M#/|M|, where inv(M), M# and |M| denote the inverse of M,
...
//
// The SSE code for the 4x4 float and double matrix inverse in former (deprecated)
// \src\LU\Inverse_SSE.h comes from the following Intel's library:
...
struct compute_inverse_size4<Architecture::Target, float, MatrixType, ResultType>
{
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        AB = psub(
            AB,
            pmul(vec4f_swizzle2(A, A, 1, 1, 2, 2), vec4f_swizzle2(B, B, 2, 3, 0, 1)));

        // DC = D#*C
        DC = pmul(vec4f_swizzle2(D, D, 3, 3, 0, 0), C);
        DC = psub(
            DC,
            pmul(vec4f_swizzle2(D, D, 1, 1, 2, 2), vec4f_swizzle2(C, C, 2, 3, 0, 1)));
        ...
        iB = psub(
            iB,
            pmul(vec4f_swizzle2(D, D, 1, 0, 3, 2), vec4f_swizzle2(AB, AB, 2, 1, 2, 1)));
        ...
        iC = psub(
            iC,
            pmul(vec4f_swizzle2(A, A, 1, 0, 3, 2), vec4f_swizzle2(DC, DC, 2, 1, 2, 1)));
...
struct compute_inverse_size4<Architecture::Target, double, MatrixType, ResultType>
{
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        // Four 2x2 sub-matrices of the input matrix, each is further divided into upper
        // and lower row e.g. A1, upper row of A, A2, lower row of A input = [[A, B],  =
        // [[[A1, [B1,
        //          [C, D]]        A2], B2]],
        //                       [[C1, [D1,
        //                         C2], D2]]]
