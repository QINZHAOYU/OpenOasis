...
template <
    typename MatrixType, typename ResultType, int Size = MatrixType::RowsAtCompileTime>
...
struct compute_inverse_and_det_with_check<MatrixType, ResultType, 1>
{
    ...
    run(const MatrixType                      &matrix,
        ...
        ResultType &result, typename ResultType::Scalar &determinant, bool &invertible)
    {
        using std::abs;
...
struct compute_inverse<MatrixType, ResultType, 4>
    : compute_inverse_size4<
          Architecture::Target, typename MatrixType::Scalar, MatrixType, ResultType>
{};
    ...
    Dense2Dense>
{
            ...
            &)
    {
        ...
        const int Size = EIGEN_PLAIN_ENUM_MIN(
            XprType::ColsAtCompileTime, DstXprType::ColsAtCompileTime);
        ...
        typedef
            typename internal::nested_eval<XprType, XprType::ColsAtCompileTime>::type
                ActualXprType;
        typedef
            typename internal::remove_all<ActualXprType>::type ActualXprTypeCleanded;
 ...
 *
 * \note This matrix must be invertible, otherwise the result is undefined. If you need
 * an invertibility check, do the following: \li for fixed sizes up to 4x4, use
 * computeInverseAndDetWithCheck(). \li for the general case, use class FullPivLU.
 *
...
{
    EIGEN_STATIC_ASSERT(
        !NumTraits<Scalar>::IsInteger, THIS_FUNCTION_IS_NOT_FOR_INTEGER_NUMERIC_TYPES)
 ...
 *
 * Notice that it will trigger a copy of input matrix when trying to do the inverse in
 * place.
 *
 * \param inverse Reference to the matrix in which to store the inverse.
 * \param determinant Reference to the variable in which to store the determinant.
 * \param invertible Reference to the bool variable in which to store whether the matrix
 * is invertible. \param absDeterminantThreshold Optional parameter controlling the
 * invertibility check. The matrix will be declared invertible if the absolute value of
 * its determinant is greater than this threshold.
 *
...
{
    // i'd love to put some static assertions there, but SFINAE means that they have no
    // effect...
    eigen_assert(rows() == cols());
