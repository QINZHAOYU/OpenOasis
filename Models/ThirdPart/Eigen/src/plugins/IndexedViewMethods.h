...
{
    typedef IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived, typename IvcRowType<RowIndices>::type,
    ...
    EIGEN_INDEXED_VIEW_METHOD_CONST
{
    return Base::operator()(
        internal::eval_expr_given_size(rowIndices, rows()),
...
IndexedView<
    EIGEN_INDEXED_VIEW_METHOD_CONST Derived, const RowIndicesT (&)[RowIndicesN],
    typename IvcColType<ColIndices>::type>
operator()(const RowIndicesT (&rowIndices)[RowIndicesN], const ColIndices &colIndices)
    EIGEN_INDEXED_VIEW_METHOD_CONST
{
    return IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived,
...
template <
    typename RowIndicesT, std::size_t RowIndicesN, typename ColIndicesT,
...
typename internal::enable_if<
    ...
    IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived, IvcIndex,
...
operator()(const Indices &indices) EIGEN_INDEXED_VIEW_METHOD_CONST
{
    ...
    return IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived,
        IvcIndex,
        typename IvcType<Indices>::type>(derived(), IvcIndex(0), ivcCol(indices));
}
...
typename internal::enable_if<
    (!IsRowMajor)
        && (!(
            ...
            || internal::is_valid_index_type<Indices>::value)),
    IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived, typename IvcType<Indices>::type,
        IvcIndex>>::type
operator()(const Indices &indices) EIGEN_INDEXED_VIEW_METHOD_CONST
{
    EIGEN_STATIC_ASSERT_VECTOR_ONLY(Derived)
    return IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived,
        typename IvcType<Indices>::type,
        IvcIndex>(derived(), ivcRow(indices), IvcIndex(0));
}
...
typename internal::enable_if<
    symbolic::is_symbolic<IndexType>::value, CoeffReturnType>::type
...
operator()(const IndicesT (&indices)[IndicesN]) EIGEN_INDEXED_VIEW_METHOD_CONST
{
    ...
    return IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived,
        IvcIndex,
        const IndicesT(&)[IndicesN]>(derived(), IvcIndex(0), indices);
}
...
operator()(const IndicesT (&indices)[IndicesN]) EIGEN_INDEXED_VIEW_METHOD_CONST
{
    ...
    return IndexedView<
        EIGEN_INDEXED_VIEW_METHOD_CONST Derived,
        const IndicesT(&)[IndicesN],
        IvcIndex>(derived(), indices, IvcIndex(0));
}
 ...
 * \endcode
 * where \c <integral \c type>  stands for any integer type compatible with Eigen::Index
 * (i.e. \c std::ptrdiff_t).
 *
 * The last statement implies compatibility with \c std::vector, \c std::valarray, \c
 * std::array, many of the Range-v3's ranges, etc.
 *
 * If the submatrix can be represented using a starting position \c (i,j) and positive
 * sizes \c (rows,columns), then this method will returns a Block object after
 * extraction of the relevant information from the passed arguments. This is the case
 * when all arguments are either:
 *  - An integer
 *  - Eigen::all
 *  - An ArithmeticSequence with compile-time increment strictly equal to 1, as returned
 * by Eigen::seq(a,b), and Eigen::seqN(a,N).
 *
 * Otherwise a more general IndexedView<Derived,RowIndices',ColIndices'> object will be
 * returned, after conversion of the inputs to more suitable types \c RowIndices' and \c
 * ColIndices'.
 *
 * For 1D vectors and arrays, you better use the operator()(const Indices&) overload,
 * which behave the same way but taking a single parameter.
 *
 * See also this <a
...
/** This is an overload of operator()(const RowIndices&, const ColIndices&) for 1D
 * vectors or arrays
 *
 * \only_for_vectors
 */
