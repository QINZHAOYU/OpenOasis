...
///
/// \param nRows the number of rows in the reshaped expression, specified at either
/// run-time or compile-time, or AutoSize \param nCols the number of columns in the
/// reshaped expression, specified at either run-time or compile-time, or AutoSize
...
///               or follows the \em natural order of the nested expression (AutoOrder).
///               The default is ColMajor.
/// \tparam NRowsType the type of the value handling the number of rows, typically
/// Index. \tparam NColsType the type of the value handling the number of columns,
/// typically Index.
///
...
/// Eigen::Dynamic. Here is an example with a fixed number of rows and columns: \include
/// MatrixBase_reshaped_fixed.cpp Output: \verbinclude MatrixBase_reshaped_fixed.out
///
/// Finally, one of the sizes parameter can be automatically deduced from the other one
/// by passing AutoSize as in the following example: \include
/// MatrixBase_reshaped_auto.cpp Output: \verbinclude MatrixBase_reshaped_auto.out
/// AutoSize does preserve compile-time sizes when possible, i.e., when the sizes of the
/// input are known at compile time \b and that the other size is passed at compile-time
/// using Eigen::fix<N> as above.
///
...
/// \returns an expression of \c *this with columns (or rows) stacked to a linear column
/// vector
///
/// \tparam Order specifies whether the coefficients should be processed in
/// column-major-order (ColMajor), in row-major-order (RowMajor),
...
///
/// - If `Order==ColMajor` (the default), then it returns a column-vector from the
/// stacked columns of \c *this.
/// - If `Order==RowMajor`, then it returns a column-vector from the stacked rows of \c
/// *this.
/// - If `Order==AutoOrder`, then it returns a column-vector with elements stacked
/// following the storage order of \c *this.
///   This mode is the recommended one when the particular ordering of the element is
///   not relevant.
///
/// Example:
...
reshaped(NRowsType nRows, NColsType nCols) EIGEN_RESHAPED_METHOD_CONST
{
    return Reshaped<
        EIGEN_RESHAPED_METHOD_CONST Derived,
        internal::
            get_compiletime_reshape_size<NRowsType, NColsType, SizeAtCompileTime>::
                value,
        internal::
            get_compiletime_reshape_size<NColsType, NRowsType, SizeAtCompileTime>::
                value>(
        derived(),
