...
template <
    typename TransformType, typename MatrixType,
...
template <
    typename Other, int Mode, int Options, int Dim, int HDim,
    int OtherRows = Other::RowsAtCompileTime, int OtherCols = Other::ColsAtCompileTime>
 ...
 *              - #AffineCompact: the transformation is stored as a (Dim)x(Dim+1)
 *matrix.
 *              - #Projective: the transformation is stored as a (Dim+1)^2 matrix
 *                             without any assumption.
 *              - #Isometry: same as #Affine with the additional assumption that
 *                           the linear part represents a rotation. This assumption is
 *exploited to speed up some functions such as inverse() and rotation(). \tparam
 *_Options has the same meaning as in class Matrix. It allows to specify DontAlign
 *and/or RowMajor. These Options are passed directly to the underlying matrix type.
 *
...
public:
    ...
    typedef Block<
        MatrixType, Dim, 1, !(internal::traits<MatrixType>::Flags & RowMajorBit)>
        TranslationPart;
    /** type of a read reference to the translation part of the rotation */
    typedef const Block<
        ConstMatrixType, Dim, 1, !(internal::traits<MatrixType>::Flags & RowMajorBit)>
...
public:
    ...
    EIGEN_DEVICE_FUNC inline Transform()
    {
        ...
        internal::transform_make_affine<
            (int(Mode) == Affine || int(Mode) == Isometry) ? Affine : AffineCompact>::
            run(m_matrix);
    }
    ...
    EIGEN_DEVICE_FUNC inline explicit Transform(const EigenBase<OtherDerived> &other)
    {
        ...
        check_template_params();
        internal::
            transform_construct_from_matrix<OtherDerived, Mode, Options, Dim, HDim>::
                run(this, other.derived());
    }
        ...
        const Transform<Scalar, Dim, OtherMode, OtherOptions> &other)
    {
        ...
        EIGEN_STATIC_ASSERT(
            EIGEN_IMPLIES(
                OtherMode == int(Affine) || OtherMode == int(AffineCompact),
                Mode != int(Isometry)),
            YOU_PERFORMED_AN_INVALID_TRANSFORMATION_CONVERSION)
        ...
        else if (EIGEN_CONST_CONDITIONAL(OtherModeIsAffineCompact))
        {
            typedef typename Transform<Scalar, Dim, OtherMode, OtherOptions>::MatrixType
                OtherMatrixType;
            internal::transform_construct_from_matrix<
                OtherMatrixType,
                Mode,
                Options,
                Dim,
        ...
        else
        {
            ...
            // if OtherMode were Projective, the static assert above would already have
            // caught it. So the only possibility is that OtherMode == Affine
            linear()      = other.linear();
     ...
     * Mode!=Projective), then \a other can also be: \li a point of size Dim (computes:
     * \code this->linear() * other + this->translation()\endcode), \li a set of N
     * points as a Dim x N matrix (computes: \code (this->linear() * other).colwise() +
     * this->translation()\endcode),
     *
     * In all cases, the return type is a matrix or vector of same sizes as the
     * right-hand-side \a other.
     *
     * If you want to interpret \a other as a linear or affine transformation, then
     * first convert it to a Transform<> type, or do your own cooking.
     *
     * Finally, if you want to apply Affine transformations to vectors, then explicitly
     * apply the linear part only: \code Affine3f A; Vector3f v1, v2; v2 = A.linear() *
     * v1; \endcode
     *
     */
    ...
    friend EIGEN_DEVICE_FUNC inline const typename internal::
        transform_left_product_impl<
            OtherDerived, Mode, Options, _Dim, _Dim + 1>::ResultType
        operator*(const EigenBase<OtherDerived> &a, const Transform &b)
    {
        return internal::
            transform_left_product_impl<OtherDerived, Mode, Options, Dim, HDim>::run(
                a.derived(), b);
    }
    ...
    {
        return internal::transform_transform_product_impl<Transform, Transform>::run(
            *this, other);
    }
...
private:
    ...
    //  (the meaning of a name may have changed since the template declaration -- the
    //  type of the template is:
    // "Eigen::internal::transform_transform_product_impl<Eigen::Transform<double, 3,
    // 32, 0>,
...
public:
    ...
    typedef typename internal::conditional<
        int(Mode) == Isometry, ConstLinearPart, const LinearMatrixType>::type
                                         RotationReturnType;
    ...
    EIGEN_DEVICE_FUNC Transform &fromPositionOrientationScale(
        const MatrixBase<PositionDerived> &position, const OrientationType &orientation,
     ...
     */
    EIGEN_DEVICE_FUNC inline Block<
 ...
 * the scaling being not necessarily positive.
 *
...
template <typename Scalar, int Dim, int Mode, int Options>
template <typename RotationMatrixType, typename ScalingMatrixType>
EIGEN_DEVICE_FUNC void Transform<Scalar, Dim, Mode, Options>::computeRotationScaling(
    RotationMatrixType *rotation, ScalingMatrixType *scaling) const
{
...
/** decomposes the linear part of the transformation as a product scaling x rotation,
 * the scaling being not necessarily positive.
 *
 * If either pointer is zero, the corresponding computation is skipped.
 *
...
{
    typedef
        typename Transform<Scalar, Dim, AffineCompact, Options>::MatrixType MatrixType;
...
struct transform_construct_from_matrix<
    Other, AffineCompact, Options, Dim, HDim, HDim, HDim>
{
    static inline void
...
struct transform_right_product_impl<TransformType, MatrixType, 2, 1>  // rhs is a vector
                                                                      // of size Dim
{
...
template <
    typename Scalar, int Dim, int LhsMode, int LhsOptions, int RhsMode, int RhsOptions>
struct transform_transform_product_impl<
