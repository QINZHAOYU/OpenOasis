 ...
 *
 * \warning When setting up an AngleAxis object, the axis vector \b must \b be \b
 * normalized.
 *
 ...
 * but rather to make easier the creation of other rotation (Quaternion, rotation
 * Matrix) and transformation objects.
 *
...
public:
    ...
    EIGEN_DEVICE_FUNC inline AngleAxis(
        const Scalar &angle, const MatrixBase<Derived> &axis) :
        m_axis(axis),
        m_angle(angle)
    {}
    ...
    EIGEN_DEVICE_FUNC inline
        typename internal::cast_return_type<AngleAxis, AngleAxis<NewScalarType>>::type
        cast() const
    {
        return typename internal::
            cast_return_type<AngleAxis, AngleAxis<NewScalarType>>::type(*this);
    }
