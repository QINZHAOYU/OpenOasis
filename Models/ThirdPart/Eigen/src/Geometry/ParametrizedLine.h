...
public:
    ...
    /** \returns the squared distance of a point \a p to its projection onto the line \c
     * *this. \sa distance()
     */
    ...
    cast() const
    {
        return typename internal::cast_return_type<
            ParametrizedLine,
            ParametrizedLine<NewScalarType, AmbientDimAtCompileTime, Options>>::
            type(*this);
    }
    ...
    EIGEN_DEVICE_FUNC inline explicit ParametrizedLine(
        const ParametrizedLine<OtherScalarType, AmbientDimAtCompileTime, OtherOptions>
            &other)
    {
    ...
    EIGEN_DEVICE_FUNC bool isApprox(
        const ParametrizedLine &other, const typename NumTraits<Scalar>::Real &prec =
 ...
 *
 * \warning the ambient space must have dimension 2 such that the hyperplane actually
 * describes a line
 */
template <typename _Scalar, int _AmbientDim, int _Options>
template <int OtherOptions>
...
/** \returns the parameter value of the intersection between \c *this and the given \a
 * hyperplane
 */
template <typename _Scalar, int _AmbientDim, int _Options>
template <int OtherOptions>
