 ...
 * transformation, but rather to make easier the constructions and updates of Transform
 * objects.
 *
 * To represent an axis aligned scaling, use the DiagonalMatrix class.
 *
 * \sa Scaling(), class DiagonalMatrix, MatrixBase::asDiagonal(), class Translation,
 * class Transform
 */
...
public:
    ...
    inline typename internal::uniformscaling_times_affine_returntype<
        Scalar, Dim, Mode>::type
    operator*(const Transform<Scalar, Dim, Mode, Options> &t) const
    {
        typename internal::uniformscaling_times_affine_returntype<Scalar, Dim, Mode>::
            type res = t;
        res.prescale(factor());
        return res;
    }
    ...
    bool isApprox(
        const UniformScaling &other, const typename NumTraits<Scalar>::Real &prec =
