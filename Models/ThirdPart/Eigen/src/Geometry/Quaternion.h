...
{
template <
    typename Other, int OtherRows = Other::RowsAtCompileTime,
...
public:
    ...
    // disabled this copy operator as it is giving very strange compilation errors when
    // compiling test_stdvector with GCC 4.4.2. This looks like a GCC bug though, so
    // feel free to re-enable it if it's useful; however notice that we already have the
    // templated operator= above and e.g. in MatrixBase we didn't have to add, in
    // addition to templated operator=, such a non-templated copy operator.
    ...
    /** \returns true if each coefficients of \c *this and \a other are all exactly
     * equal. \warning When using floating point scalar values you probably should
    ...
    /** \returns true if at least one pair of coefficients of \c *this and \a other are
     * not exactly equal to each other. \warning When using floating point scalar values
     * you probably should rather use a fuzzy comparison such as isApprox() \sa
     * isApprox(), operator== */
    template <class OtherDerived>
    EIGEN_DEVICE_FUNC inline bool
    operator!=(const QuaternionBase<OtherDerived> &other) const
    ...
    EIGEN_DEVICE_FUNC inline
        typename internal::cast_return_type<Derived, Quaternion<NewScalarType>>::type
        cast() const;
 ...
 * representation of orientations and rotations of objects in three dimensions. Compared
 * to other representations like Euler angles or 3x3 matrices, quaternions offer the
 * following advantages: \li \b compact storage (4 scalars) \li \b efficient to compose
 * (28 flops), \li \b stable spherical interpolation
 *
 * The following two typedefs are provided for convenience:
 * \li \c Quaternionf for \c float
 * \li \c Quaterniond for \c double
 *
 * \warning Operations interpreting the quaternion as rotation have undefined behavior
...
protected:
    ...
    {
        EIGEN_STATIC_ASSERT(
            (_Options & DontAlign) == _Options, INVALID_MATRIX_TEMPLATE_PARAMETERS)
    }
#endif
};
...
public:
     ...
     *
     * The pointer \a coeffs must reference the four coefficients of Quaternion in the
     * following order: \code *coeffs == {x, y, z, w} \endcode
     *
     * If the template parameter _Options is set to #Aligned, then the pointer coeffs
     * must be aligned. */
...
// This product can be specialized for a given architecture via the Arch template
// argument.
namespace internal
{
...
{
    EIGEN_STATIC_ASSERT(
        (internal::is_same<typename Derived::Scalar, typename OtherDerived::Scalar>::
             value),
...
QuaternionBase<Derived>::operator=(const AngleAxisType &aa)
{
    ...
    EIGEN_USING_STD(sin)
    Scalar ha =
        Scalar(0.5) * aa.angle();  // Scalar(0.5) to suppress precision loss warnings
    this->w()   = cos(ha);
...
{
    EIGEN_STATIC_ASSERT(
        (internal::is_same<typename Derived::Scalar, typename MatrixDerived::Scalar>::
             value),
        YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
...
EIGEN_DEVICE_FUNC Quaternion<Scalar, Options>
                  Quaternion<Scalar, Options>::FromTwoVectors(
    const MatrixBase<Derived1> &a, const MatrixBase<Derived2> &b)
{
    Quaternion quat;
...
{
    // FIXME should this function be called multiplicativeInverse and conjugate() be
    // called inverse() or opposite()  ??
...
struct quaternionbase_assign_impl<Other, 3, 3>
{
    ...
    run(QuaternionBase<Derived> &q, const Other &a_mat)
    {
...
struct quaternionbase_assign_impl<Other, 4, 1>
{
    ...
    template <class Derived>
    EIGEN_DEVICE_FUNC static inline void
    run(QuaternionBase<Derived> &q, const Other &vec)
    {
        q.coeffs() = vec;
    }
};
