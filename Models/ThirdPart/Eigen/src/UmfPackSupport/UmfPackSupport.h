...
inline void umfpack_report_info(
    double control[UMFPACK_CONTROL], double info[UMFPACK_INFO], double, int)
{
    umfpack_di_report_info(control, info);
}

inline void umfpack_report_info(
    double control[UMFPACK_CONTROL], double info[UMFPACK_INFO], std::complex<double>,
    int)
{
...
inline void umfpack_report_info(
    double control[UMFPACK_CONTROL], double info[UMFPACK_INFO], double,
    SuiteSparse_long)
{
...
inline void umfpack_report_status(
    double control[UMFPACK_CONTROL], int status, std::complex<double>, int)
{
    umfpack_zi_report_status(control, status);
}

inline void umfpack_report_status(
    double control[UMFPACK_CONTROL], int status, double, SuiteSparse_long)
{
    umfpack_dl_report_status(control, status);
}

inline void umfpack_report_status(
    double control[UMFPACK_CONTROL], int status, std::complex<double>, SuiteSparse_long)
{
...
inline int umfpack_symbolic(
    int n_row, int n_col, const int Ap[], const int Ai[],
    const std::complex<double> Ax[], void **Symbolic,
    const double Control[UMFPACK_CONTROL], double Info[UMFPACK_INFO])
{
    return umfpack_zi_symbolic(
        n_row, n_col, Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Control, Info);
}
...
inline SuiteSparse_long umfpack_symbolic(
    ...
    const double Control[UMFPACK_CONTROL], double Info[UMFPACK_INFO])
{
    return umfpack_zl_symbolic(
        n_row, n_col, Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Control, Info);
}

// Numeric
inline int umfpack_numeric(
    const int Ap[], const int Ai[], const double Ax[], void *Symbolic, void **Numeric,
    const double Control[UMFPACK_CONTROL], double Info[UMFPACK_INFO])
{
    ...
    void **Numeric, const double Control[UMFPACK_CONTROL], double Info[UMFPACK_INFO])
{
    return umfpack_zi_numeric(
        Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Numeric, Control, Info);
}
inline SuiteSparse_long umfpack_numeric(
    const SuiteSparse_long Ap[], const SuiteSparse_long Ai[], const double Ax[],
...
inline SuiteSparse_long umfpack_numeric(
    ...
    const double Control[UMFPACK_CONTROL], double Info[UMFPACK_INFO])
{
    return umfpack_zl_numeric(
        Ap, Ai, &numext::real_ref(Ax[0]), 0, Symbolic, Numeric, Control, Info);
}

// solve
inline int umfpack_solve(
    int sys, const int Ap[], const int Ai[], const double Ax[], double X[],
    const double B[], void *Numeric, const double Control[UMFPACK_CONTROL],
    double Info[UMFPACK_INFO])
{
...
inline int umfpack_solve(
    int sys, const int Ap[], const int Ai[], const std::complex<double> Ax[],
...
inline int umfpack_get_lunz(
    int *lnz, int *unz, int *n_row, int *n_col, int *nz_udiag, void *Numeric, double)
{
    return umfpack_di_get_lunz(lnz, unz, n_row, n_col, nz_udiag, Numeric);
}

inline int umfpack_get_lunz(
    int *lnz, int *unz, int *n_row, int *n_col, int *nz_udiag, void *Numeric,
    std::complex<double>)
{
...
inline SuiteSparse_long umfpack_get_lunz(
    SuiteSparse_long *lnz, SuiteSparse_long *unz, SuiteSparse_long *n_row,
...
inline int umfpack_get_numeric(
    int Lp[], int Lj[], double Lx[], int Up[], int Ui[], double Ux[], int P[], int Q[],
    double Dx[], int *do_recip, double Rs[], void *Numeric)
{
    return umfpack_di_get_numeric(
        Lp, Lj, Lx, Up, Ui, Ux, P, Q, Dx, do_recip, Rs, Numeric);
}

inline int umfpack_get_numeric(
    int Lp[], int Lj[], std::complex<double> Lx[], int Up[], int Ui[],
    std::complex<double> Ux[], int P[], int Q[], std::complex<double> Dx[],
...
inline SuiteSparse_long umfpack_get_numeric(
    SuiteSparse_long Lp[], SuiteSparse_long Lj[], double Lx[], SuiteSparse_long Up[],
    SuiteSparse_long Ui[], double Ux[], SuiteSparse_long P[], SuiteSparse_long Q[],
    double Dx[], SuiteSparse_long *do_recip, double Rs[], void *Numeric)
{
    return umfpack_dl_get_numeric(
        Lp, Lj, Lx, Up, Ui, Ux, P, Q, Dx, do_recip, Rs, Numeric);
}
...
inline int umfpack_get_determinant(
    double *Mx, double *Ex, void *NumericHandle, double User_Info[UMFPACK_INFO], int)
{
    return umfpack_di_get_determinant(Mx, Ex, NumericHandle, User_Info);
}

inline int umfpack_get_determinant(
    std::complex<double> *Mx, double *Ex, void *NumericHandle,
...
inline SuiteSparse_long umfpack_get_determinant(
    double *Mx, double *Ex, void *NumericHandle, double User_Info[UMFPACK_INFO],
    SuiteSparse_long)
{
 ...
 * makeCompressed() to get a compressed matrix. \tparam _MatrixType the type of the
 * sparse matrix A, it must be a SparseMatrix<>
 *
...
public:
    ...
    UmfPackLU() : m_dummy(0, 0), mp_matrix(m_dummy)
    {
        init();
    }
     ...
     *  Note that the matrix should be column-major, and in compressed format for best
     * performance. \sa SparseMatrix::makeCompressed().
     */
    template <typename InputMatrixType>
     ...
     *
     * This function is particularly useful when solving for several problems having the
     * same structure.
     *
     ...
     *
     * The given matrix must has the same sparcity than the matrix on which the pattern
     * anylysis has been performed.
     *
    ...
    {
        eigen_assert(
            m_analysisIsOk && "UmfPackLU: you must first call analyzePattern()");
        if (m_numeric)
            umfpack_free_numeric(&m_numeric, Scalar(), StorageIndex());
    ...
    void printUmfpackInfo()
    {
        eigen_assert(
            m_analysisIsOk && "UmfPackLU: you must first call analyzePattern()");
        umfpack_report_info(
            m_control.data(), m_umfpackInfo.data(), Scalar(), StorageIndex());
    }

    /** Prints the status of the previous factorization operation performed by UmfPack
     * (symbolic or numerical factorization).
     *
     * \sa analyzePattern(), compute()
     */
    void printUmfpackStatus()
    {
        ...
        umfpack_report_status(
            m_control.data(), m_fact_errorCode, Scalar(), StorageIndex());
    }
...
template <typename MatrixType>
template <typename BDerived, typename XDerived>
bool UmfPackLU<MatrixType>::_solve_impl(
    const MatrixBase<BDerived> &b, MatrixBase<XDerived> &x) const
{
    Index rhsCols = b.cols();
    eigen_assert(
        (BDerived::Flags & RowMajorBit) == 0
        && "UmfPackLU backend does not support non col-major rhs yet");
    eigen_assert(
        (XDerived::Flags & RowMajorBit) == 0
        && "UmfPackLU backend does not support non col-major result yet");
    eigen_assert(
        b.derived().data() != x.derived().data()
