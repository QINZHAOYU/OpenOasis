...
struct bitwise_helper<
    T, typename internal::enable_if<
           is_scalar<T>::value
           && (NumTraits<T>::IsInteger || NumTraits<T>::RequireInitialization)>::type>
...
struct pselect_impl
{
    static EIGEN_DEVICE_FUNC inline Packet
    run(const Packet &mask, const Packet &a, const Packet &b)
    {
...
/** \internal \returns the significant and exponent of the underlying floating point
 * numbers See https://en.cppreference.com/w/cpp/numeric/math/frexp
 */
template <typename Packet>
...
EIGEN_DEVICE_FUNC inline void pbroadcast4(
    const typename unpacket_traits<Packet>::type *a, Packet &a0, Packet &a1, Packet &a2,
    Packet &a3)
{
...
/** \internal \returns the sum of the elements of upper and lower half of \a a if \a a
 * is larger than 4. For a packet {a0, a1, a2, a3, a4, a5, a6, a7}, it returns a half
 * packet {a0+a4, a1+a5, a2+a6, a3+a7} For packet-size smaller or equal to 4, this boils
 * down to a noop.
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline typename conditional<
    (unpacket_traits<Packet>::size % 8) == 0, typename unpacket_traits<Packet>::half,
    Packet>::type
...
{
    typedef typename unpacket_traits<Packet>::type Scalar;
    return predux_helper(
        a, EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmin<PropagateFast, Scalar>)));
}
...
predux_min(const Packet &a)
{
    typedef typename unpacket_traits<Packet>::type Scalar;
    return predux_helper(
        a, EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmin<NaNPropagation, Scalar>)));
}
...
predux_max(const Packet &a)
{
    typedef typename unpacket_traits<Packet>::type Scalar;
    return predux_helper(
        a, EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmax<PropagateFast, Scalar>)));
}
...
predux_max(const Packet &a)
{
    typedef typename unpacket_traits<Packet>::type Scalar;
    return predux_helper(
        a, EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmax<NaNPropagation, Scalar>)));
}
...
{
    // Dirty but generic implementation where "true" is assumed to be non 0 and all the
    // sames. It is expected that "true" is either:
    //  - Scalar(1)
    //  - bits full of ones (NaN for floats),
    //  - or first bit equals to 1 (1 for ints, smallest denormal for floats).
    // For all these cases, taking the sum is just fine, and this boils down to a no-op
    // for scalars.
    typedef typename unpacket_traits<Packet>::type Scalar;
...
/** \internal copy a packet with constant coefficient \a a (e.g., [a,a,a,a]) to \a *to.
 * \a to must be 16 bytes aligned */
...
EIGEN_DEVICE_FUNC inline Packet pblend(
    const Selector<unpacket_traits<Packet>::size> &ifPacket, const Packet &thenPacket,
