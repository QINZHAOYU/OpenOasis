...
struct product_type
{
    ...
    enum
    {
        ...
        MaxDepth = EIGEN_SIZE_MIN_PREFER_FIXED(
            traits<_Lhs>::MaxColsAtCompileTime, traits<_Rhs>::MaxRowsAtCompileTime),
        Depth = EIGEN_SIZE_MIN_PREFER_FIXED(
            traits<_Lhs>::ColsAtCompileTime, traits<_Rhs>::RowsAtCompileTime)
    };

    // the splitting into different lines of code here, introducing the _select enums
    // and the typedef below, is to work around an internal compiler error with gcc 4.1
    // and 4.2.
private:
    enum
    {
...
// case, we could have a specialization for Block<MatrixType,1,1> with: operator=(Scalar
// x);
 ...
 *   1 - the matrix is col-major, BLAS compatible and M is large => call fast BLAS-like
 * colmajor routine 2 - the matrix is row-major, BLAS compatible and N is large => call
 * fast BLAS-like rowmajor routine 3 - all other cases are handled using a simple loop
 * along the outer-storage direction. Therefore we need a lower level meta selector.
...
struct gemv_static_vector_if<Scalar, Size, MaxSize, true>
{
    ...
    internal::plain_array<
        Scalar, EIGEN_SIZE_MIN_PREFER_FIXED(Size, MaxSize), 0,
    ...
    run(const Lhs &lhs, const Rhs &rhs, Dest &dest, const typename Dest::Scalar &alpha)
    {
        ...
        else
        {
            ...
            if (!evalToDest)
            {
                if (!alphaIsCompatible)
                    dest.matrix() +=
                        actualAlpha * MappedDest(actualDestPtr, dest.size());
                else
...
struct gemv_dense_selector<OnTheRight, ColMajor, false>
{
    ...
    run(const Lhs &lhs, const Rhs &rhs, Dest &dest, const typename Dest::Scalar &alpha)
    {
        ...
        // TODO if rhs is large enough it might be beneficial to make sure that dest is
        // sequentially stored in memory, otherwise use a temp
...
/** \returns an expression of the matrix product of \c *this and \a other without
 * implicit evaluation.
 *
 * The returned product will behave like any other expressions: the coefficients of the
 * product will be computed once at a time as requested. This might be useful in some
 * extremely rare cases when only a small and no coherent fraction of the result's
 * coefficients have to be computed.
 *
 * \warning This version of the matrix product can be much much slower. So use it only
 * if you know what you are doing and that you measured a true speed improvement.
 *
