...
struct functor_traits<scalar_sum_op<LhsScalar, RhsScalar>>
{
    enum
    {
        Cost = (int(NumTraits<LhsScalar>::AddCost) + int(NumTraits<RhsScalar>::AddCost))
               / 2,  // rough estimate!
...
struct scalar_product_op : binary_op_base<LhsScalar, RhsScalar>
{
    typedef typename ScalarBinaryOpTraits<
        LhsScalar, RhsScalar, scalar_product_op>::ReturnType result_type;
#ifndef EIGEN_SCALAR_BINARY_OP_PLUGIN
...
struct functor_traits<scalar_product_op<LhsScalar, RhsScalar>>
{
    enum
    {
        Cost = (int(NumTraits<LhsScalar>::MulCost) + int(NumTraits<RhsScalar>::MulCost))
               / 2,  // rough estimate!
        PacketAccess = is_same<LhsScalar, RhsScalar>::value
 ...
 *
 * This is a short cut for conj(x) * y which is needed for optimization purpose; in
 * Eigen2 support mode, this becomes x * conj(y)
 */
template <typename LhsScalar, typename RhsScalar>
...
/** \internal
 * \brief Template functor to compute the hypot of two \b positive \b and \b real
 * scalars
 *
...
struct scalar_hypot_op<Scalar, Scalar> : binary_op_base<Scalar, Scalar>
{
    ...
    {
        // This functor is used by hypotNorm only for which it is faster to first apply
        // abs on all coefficients prior to reduction through hypot. This way we avoid
        // calling abs on positive and real entries, and this also permits to seamlessly
        // handle complexes. Otherwise we would have to handle both real and complexes
...
{
    typedef typename ScalarBinaryOpTraits<Scalar, Exponent, scalar_pow_op>::ReturnType
        result_type;
#ifndef EIGEN_SCALAR_BINARY_OP_PLUGIN
...
{
    typedef typename ScalarBinaryOpTraits<
        LhsScalar, RhsScalar, scalar_quotient_op>::ReturnType result_type;
#ifndef EIGEN_SCALAR_BINARY_OP_PLUGIN
...
{
    typedef typename ScalarBinaryOpTraits<
        LhsScalar, RhsScalar, scalar_absolute_difference_op>::ReturnType result_type;
#ifndef EIGEN_SCALAR_BINARY_OP_PLUGIN
...
struct functor_traits<scalar_absolute_difference_op<LhsScalar, RhsScalar>>
{
    enum
    {
        Cost = (NumTraits<LhsScalar>::AddCost + NumTraits<RhsScalar>::AddCost) / 2,
        PacketAccess =
            is_same<LhsScalar, RhsScalar>::value && packet_traits<LhsScalar>::HasAbsDiff
    };
};
...
// The following two classes permits to turn any binary functor into a unary one with
// one argument bound to a constant value. They are analogues to
