...
struct functor_traits<scalar_constant_op<Scalar>>
{
    enum
    {
        Cost = 0 /* as the constant value should be loaded in register only once for the
                    whole expression */
        ,
...
struct linspaced_op_impl<Scalar, /*IsInteger*/ false>
{
    ...
    linspaced_op_impl(const Scalar &low, const Scalar &high, Index num_steps) :
        m_low(low),
        m_high(high), m_size1(num_steps == 1 ? 1 : num_steps - 1),
        m_step(
            num_steps == 1 ? Scalar() :
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Packet packetOp(IndexType i) const
    {
        ...
        else
        {
            ...
            if (EIGEN_PREDICT_TRUE(i != m_size1 - unpacket_traits<Packet>::size + 1))
                return res;
            Packet mask = pcmp_lt(
                plset<Packet>(0), pset1<Packet>(unpacket_traits<Packet>::size - 1));
...
struct linspaced_op_impl<Scalar, /*IsInteger*/ true>
{
    ...
    linspaced_op_impl(const Scalar &low, const Scalar &high, Index num_steps) :
        m_low(low),
        m_multiplier(
            (high - low) / convert_index<Scalar>(num_steps <= 1 ? 1 : num_steps - 1)),
        m_divisor(
            convert_index<Scalar>((high >= low ? num_steps : -num_steps) + (high - low))
...
// Linear access is automatically determined from the operator() prototypes available
// for the given functor. If it exposes an operator()(i,j), then we assume the i and j
// coefficients are required independently and linear access is not possible. In all
// other cases, linear access is enabled. Users should not have to deal with this
// structure.
...
#if !(                                                                                 \
    (EIGEN_COMP_MSVC > 1600) || (EIGEN_GNUC_AT_LEAST(4, 8))                            \
    || (EIGEN_COMP_ICC >= 1600))
