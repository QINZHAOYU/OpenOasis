...
// This issue has been confirmed with MSVC 2008 only, but the issue might exist for more
// recent versions too.
 ...
 *
 * How it's used:
 * To allow to defined the global math functions (like sin...) in certain cases, like
 * the Array expressions. When you do sin(array1+array2), the object array1+array2 has a
 * complicated expression type, all what you want to know is that it inherits ArrayBase.
 * So we implement a partial specialization of sin_impl for ArrayBase<Derived>. So we
 * must make sure to use sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>,
 * otherwise our partial specialization won't be used. How does sin know that? That's
 * exactly what global_math_functions_filtering_base tells it.
 *
 * How it's implemented:
 * SFINAE in the style of enable_if. Highly susceptible of breaking compilers. With GCC,
 * it sure does work, but if you replace the typename dummy by an integer template
 * parameter, it doesn't work anymore!
 */
...
struct global_math_functions_filtering_base<
    T, typename always_void<
           typename T::Eigen_BaseClassForSpecializationOfGlobalMathFuncImpl>::type>
{
...
// here, for once, we're plainly returning NewType: we don't want cast to do weird
// things.
...
struct arg_default_impl<Scalar, true>
{
    ...
    static inline RealScalar run(const Scalar &x)
    {
        ...
        // HIP does not seem to have a native device side implementation for the math
        // routine "arg"
        using std::arg;
#else
        EIGEN_USING_STD(arg);
...
{
// fallback expm1 implementation in case there is no expm1(Scalar) function in namespace
// of Scalar, or that there is no suitable std::expm1 function available. Implementation
...
namespace std_fallback
{
// fallback log1p implementation in case there is no log1p(Scalar) function in namespace
// of Scalar, or that there is no suitable std::log1p function available
template <typename Scalar>
...
template <
    unsigned int n, int lower = 0, int upper = sizeof(unsigned int) * CHAR_BIT - 1,
...
struct meta_floor_log2<n, lower, upper, meta_floor_log2_move_up>
{
    ...
    {
        value = meta_floor_log2<
            n, meta_floor_log2_selector<n, lower, upper>::middle, upper>::value
    };
};
...
struct random_default_impl<Scalar, false, true>
{
    static inline Scalar run(const Scalar &x, const Scalar &y)
    {
        ...
        typedef
            typename conditional<(ScalarU(-1) > unsigned(-1)), ScalarU, unsigned>::type
                ScalarX;
        ...
        // unsigned variants. This is the case in most modern implementations of C++.
...
// std::is* do not work with fast-math and gcc, std::is* are available on MSVC 2013 and
// newer, as well as in clang.
...
EIGEN_DEVICE_FUNC typename internal::enable_if<
    (!internal::is_integral<T>::value) && (!NumTraits<T>::IsComplex), bool>::type
...
#else
// NOTE the inline qualifier and noinline attribute are both needed: the former is to
// avoid linking issue (duplicate symbol),
//      while the second prevent too aggressive optimizations in fast-math mode:
#define EIGEN_TMP_NOOPT_ATTRIB                                                         \
    EIGEN_DEVICE_FUNC inline __attribute__((noinline, optimize("no-finite-math-only")))
#endif
...
#define SYCL_SPECIALIZE_GEN1_BINARY_FUNC(NAME, FUNC, RET_TYPE, ARG_TYPE1, ARG_TYPE2)   \
    template <>                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE RET_TYPE NAME(                               \
        const ARG_TYPE1 &x, const ARG_TYPE2 &y)                                        \
    {                                                                                  \
...
template <typename ScalarX, typename ScalarY>
EIGEN_DEVICE_FUNC inline typename internal::pow_impl<ScalarX, ScalarY>::result_type
...
template <typename T>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename internal::enable_if<
    NumTraits<T>::IsSigned || NumTraits<T>::IsComplex,
