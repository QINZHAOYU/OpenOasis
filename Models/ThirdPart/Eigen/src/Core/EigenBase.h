 ...
 * \ingroup Core_Module
 *
 * Common base class for all classes T such that MatrixBase has an operator=(T) and a
 * constructor MatrixBase(T).
 *
 * In other words, an EigenBase object is an object that can be copied into a
 * MatrixBase.
 *
 * Besides MatrixBase-derived classes, this also includes special matrix classes such as
 * diagonal matrices, etc.
 *
...
struct EigenBase
{
     ...
     * deprecated. Use Eigen::Index instead. Deprecation is not marked with a doxygen
     * comment because there are too many existing usages to add the deprecation
     * attribute.
     */
    ...
    /** \internal Don't use it, but do the equivalent: \code dst.applyOnTheRight(*this);
     * \endcode */
    template <typename Dest>
    ...
    /** \internal Don't use it, but do the equivalent: \code dst.applyOnTheLeft(*this);
     * \endcode */
    template <typename Dest>
