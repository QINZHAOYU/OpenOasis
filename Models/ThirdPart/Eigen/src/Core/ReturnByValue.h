    ...
    : public traits<typename traits<Derived>::ReturnType>
{
    enum
    {
        ...
        Flags =
            (traits<typename traits<Derived>::ReturnType>::Flags | EvalBeforeNestingBit)
            & ~DirectAccessBit
    };
};

/* The ReturnByValue object doesn't even have a coeff() method.
 * So the only way that nesting it in an expression can work, is by evaluating it into a
 * plain matrix. So internal::nested always gives the plain return matrix type.
 *
 * FIXME: I don't understand why we need this specialization: isn't this taken care of
 * by the EvalBeforeNestingBit ?? Answer: EvalBeforeNestingBit should be deprecated
 * since we have the evaluators
 */
...
// Expression is evaluated in a temporary; default implementation of Assignment is
// bypassed so that when a ReturnByValue expression is assigned, the evaluator is not
// constructed.
// TODO: Finalize port to new regime; ReturnByValue should not exist in the expression
// world
