...
struct packetwise_redux_impl<Func, Evaluator, CompleteUnrolling>
{
    ...
    {
        return redux_vec_unroller<
            Func,
            Evaluator,
            0,
            packetwise_redux_traits<Func, Evaluator>::OuterSize>::
...
struct packetwise_redux_impl<Func, Evaluator, NoUnrolling>
{
    ...
    template <typename PacketType>
    EIGEN_DEVICE_FUNC static PacketType
    run(const Evaluator &eval, const Func &func, Index size)
    {
        if (size == 0)
        ...
        // - thanks to branch prediction and out-of-order execution we have independent
        // instructions across loops
        for (; i < size4; i += 4)
            p = func.packetOp(
                p,
                func.packetOp(
    ...
    : evaluator_base<PartialReduxExpr<ArgType, MemberOp, Direction>>
{
    ...
    enum
    {
        ...
        Alignment = 0  // FIXME this will need to be improved once PartialReduxExpr is
                       // vectorized
    };
    ...
    EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC PacketType packet(Index idx) const
    {
        ...
        // FIXME
        // See bug 1612, currently if PacketSize==1 (i.e. complex<double> with 128bits
        // registers) then the storage-order of panel get reversed and methods like
        // packetByOuterInner do not make sense anymore in this context. So let's just
