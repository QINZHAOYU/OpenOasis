...
EIGEN_ALWAYS_INLINE void storeAccumulator(
    Index i, Index j, const DataMapper &data, const Packet &alpha, __vector_quad *acc)
{
...
template <
    typename DataMapper, typename Index, typename Packet, typename Packetc,
    const Index accColsC, int N>
EIGEN_ALWAYS_INLINE void storeComplexAccumulator(
    Index i, Index j, const DataMapper &data, const Packet &alphaReal,
    const Packet &alphaImag, __vector_quad *accReal, __vector_quad *accImag)
{
    ...
    bscalec<Packet, 4>(
        resultReal, resultImag, alphaReal, alphaImag, taccReal, taccImag);
...
// Defaults to float32, since Eigen still supports C++03 we can't use default template
// arguments
template <typename LhsPacket, typename RhsPacket, bool NegativeAccumulate>
EIGEN_ALWAYS_INLINE void
pgerMMA(__vector_quad *acc, const RhsPacket &a, const LhsPacket &b)
{
    if (NegativeAccumulate)
    {
        __builtin_mma_xvf32gernp(
            acc, (__vector unsigned char)a, (__vector unsigned char)b);
    }
    else
    {
...
template <typename LhsPacket, typename RhsPacket, bool NegativeAccumulate>
EIGEN_ALWAYS_INLINE void
pgerMMA(__vector_quad *acc, const PacketBlock<Packet2d, 2> &a, const Packet2d &b)
{
...
template <typename LhsPacket, typename RhsPacket, bool NegativeAccumulate>
EIGEN_ALWAYS_INLINE void
pgerMMA(__vector_quad *acc, const __vector_pair &a, const Packet2d &b)
{
    if (NegativeAccumulate)
    {
...
template <typename LhsPacket, typename RhsPacket, bool NegativeAccumulate>
EIGEN_ALWAYS_INLINE void
pgerMMA(__vector_quad *, const __vector_pair &, const Packet4f &)
{
    // Just for compilation
}

template <
    typename Scalar, typename Packet, typename RhsPacket, bool ConjugateLhs,
    bool ConjugateRhs, bool LhsIsReal, bool RhsIsReal>
EIGEN_ALWAYS_INLINE void pgercMMA(
    __vector_quad *accReal, __vector_quad *accImag, const Packet &lhsV,
    const Packet &lhsVi, const RhsPacket &rhsV, const RhsPacket &rhsVi)
{
    ...
    else
    {
        if (!RhsIsReal)
        {
            pgerMMA<Packet, RhsPacket, ConjugateLhs == ConjugateRhs>(
                accReal, rhsVi, lhsVi);
            pgerMMA<Packet, RhsPacket, ConjugateRhs>(accImag, rhsVi, lhsV);
        }
        else
...
// This is necessary because ploadRhs for double returns a pair of vectors when MMA is
// enabled.
template <typename Scalar, typename Packet>
...
template <>
EIGEN_ALWAYS_INLINE void ploadRhsMMA<double, PacketBlock<Packet2d, 2>>(
...
#define MICRO_MMA_ONE_PEEL                                                             \
    ...
    {                                                                                  \
        MICRO_MMA_UNROLL_TYPE_PEEL(                                                    \
            MICRO_MMA_WORK_ONE, MICRO_MMA_LOAD_ONE, __vector_pair);                    \
    }                                                                                  \
...
template <
    int unroll_factor, typename Scalar, typename Packet, typename RhsPacket,
    ...
    Index strideA, Index offsetA, Index &row, Index col, const Packet &pAlpha)
{
    ...
    __vector_quad accZero0, accZero1, accZero2, accZero3, accZero4, accZero5, accZero6,
        accZero7;
...
#define MICRO_COMPLEX_MMA_ONE_PEEL                                                     \
    ...
    {                                                                                  \
        MICRO_COMPLEX_MMA_UNROLL_TYPE_PEEL(                                            \
            MICRO_COMPLEX_MMA_WORK_ONE, MICRO_COMPLEX_MMA_LOAD_ONE, RhsPacket);        \
    }                                                                                  \
    else                                                                               \
    {                                                                                  \
        MICRO_COMPLEX_MMA_UNROLL_TYPE_PEEL(                                            \
...
template <
    ...
    typename RhsPacket, typename DataMapper, typename Index, const Index accRows,
    const Index accCols, bool ConjugateLhs, bool ConjugateRhs, bool LhsIsReal,
    bool RhsIsReal>
    ...
    const Packet &pAlphaReal, const Packet &pAlphaImag)
{
    ...
    __vector_quad accReal0, accImag0, accReal1, accImag1, accReal2, accImag2, accReal3,
        accImag3, accReal4, accImag4;
