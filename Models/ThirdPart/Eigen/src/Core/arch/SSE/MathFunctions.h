...
prsqrt<Packet4f>(const Packet4f &_x)
{
    ...
    Packet4f y_newton =
        pmul(y_approx, pmadd(y_approx, pmul(neg_half, y_approx), p4f_one_point_five));
...
{
    // Unfortunately we can't use the much faster mm_rsqrt_ps since it only provides an
    // approximation.
