...
EIGEN_STRONG_INLINE Packet2cf pmul<Packet2cf>(const Packet2cf &a, const Packet2cf &b)
{
    ...
    return Packet2cf(_mm_add_ps(
        ...
        _mm_xor_ps(
            _mm_mul_ps(
                vec4f_swizzle1(a.v, 1, 1, 3, 3), vec4f_swizzle1(b.v, 1, 0, 3, 2)),
            mask)));
#endif
}
    ...
    std::complex<float> *to, const Packet2cf &from, Index stride)
{
    to[stride * 0] = std::complex<float>(
        _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 0)),
        _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 1)));
    to[stride * 1] = std::complex<float>(
        _mm_cvtss_f32(_mm_shuffle_ps(from.v, from.v, 2)),
...
EIGEN_STRONG_INLINE Packet1cd pmul<Packet1cd>(const Packet1cd &a, const Packet1cd &b)
{
    ...
    return Packet1cd(_mm_add_pd(
        ...
        _mm_xor_pd(
            _mm_mul_pd(vec2d_swizzle1(a.v, 1, 1), vec2d_swizzle1(b.v, 1, 0)), mask)));
#endif
}
...
{
    __m128d result = pblend<Packet2d>(
        ifPacket, _mm_castps_pd(thenPacket.v), _mm_castps_pd(elsePacket.v));
