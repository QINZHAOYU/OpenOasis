...
public:
    ...
    cl::sycl::buffer<buffer_data_type, 1> get_buffer(const virtual_pointer_t ptr)
    {
        ...
        eigen_assert(
            ptr < static_cast<virtual_pointer_t>(node->second.m_size + node->first));
     ...
     */
    template <
        sycl_acc_mode   access_mode   = default_acc_mode,
...
private:
    ...
    {
        bool operator()(
            typename pointerMap_t::iterator a, typename pointerMap_t::iterator b) const
        {
...
struct RangeAccess
{
    ...
    RangeAccess(accessor access, size_t offset, std::intptr_t virtual_ptr) :
        access_(access),
        offset_(offset), virtual_ptr_(virtual_ptr)
    {}

    RangeAccess(
        cl::sycl::buffer<scalar_t, 1> buff =
            cl::sycl::buffer<scalar_t, 1>(cl::sycl::range<1>(1))) :
        access_{accessor{buff}},
        offset_(0), virtual_ptr_(-1)
    {}
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE scalar_t *get_virtual_pointer() const
    {
        return reinterpret_cast<scalar_t *>(
            virtual_ptr_ + (offset_ * sizeof(scalar_t)));
    }
