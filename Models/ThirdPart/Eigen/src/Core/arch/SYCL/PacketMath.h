...
#define SYCL_PLOADT_RO(address_space_target)                                           \
    ...
    {                                                                                  \
        ...
        typedef cl::sycl::                                                             \
            multi_ptr<scalar, cl::sycl::access::address_space::address_space_target>   \
                multi_ptr;                                                             \
        auto    res =                                                                  \
            packet_type(static_cast<typename unpacket_traits<packet_type>::type>(0));  \
...
ploadt(const Eigen::TensorSycl::internal::RangeAccess<
       cl::sycl::access::mode::read_write, typename unpacket_traits<packet_type>::type>
           &from)
{
...
struct get_base_packet<cl::sycl::cl_float4>
{
    ...
    set_pscatter(sycl_multi_pointer to, const cl::sycl::cl_float4 &from, Index stride)
    {
        auto tmp          = stride;
        to[0]             = from.x();
        to[tmp]           = from.y();
...
struct get_base_packet<cl::sycl::cl_double2>
{
    ...
    template <typename sycl_multi_pointer>
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
    set_pscatter(sycl_multi_pointer to, const cl::sycl::cl_double2 &from, Index stride)
    {
        to[0]      = from.x();
        to[stride] = from.y();
    }
...
sycl_pcmp_le(const Packet &a, const Packet &b)
{
    return ((a <= b)
                .template convert<
...
template <typename Packet>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet
sycl_pcmp_lt(const Packet &a, const Packet &b)
{
    return ((a < b)
                .template convert<
                    typename unpacket_traits<Packet>::type,
...
template <typename Packet>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE Packet
sycl_pcmp_eq(const Packet &a, const Packet &b)
{
    return ((a == b)
                .template convert<
                    typename unpacket_traits<Packet>::type,
...
#define SYCL_PCMP(OP, TYPE)                                                            \
    template <>                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE TYPE pcmp_##OP<TYPE>(                        \
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
    typename convert_to_integer<typename unpacket_traits<PacketIn>::type>::packet_type
    vector_as_int(const PacketIn &p)
{
    return (p.template convert<
            typename convert_to_integer<typename unpacket_traits<PacketIn>::type>::type,
            cl::sycl::rounding_mode::automatic>());
}
...
{
    return (p.template convert<
            typename unpacket_traits<packetOut>::type,
            cl::sycl::rounding_mode::automatic>());
}

#define SYCL_PAND(TYPE)                                                                \
    template <>                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pand<TYPE>(                             \
        const TYPE &a, const TYPE &b)                                                  \
    {                                                                                  \
...
#define SYCL_PXOR(TYPE)                                                                \
    template <>                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pxor<TYPE>(                             \
        const TYPE &a, const TYPE &b)                                                  \
    {                                                                                  \
        return convert_vector<TYPE>(vector_as_int(a) ^ vector_as_int(b));              \
    }
...
#define SYCL_PANDNOT(TYPE)                                                             \
    template <>                                                                        \
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TYPE pandnot<TYPE>(                          \
        const TYPE &a, const TYPE &b)                                                  \
    {                                                                                  \
        return convert_vector<TYPE>(vector_as_int(a) & (~vector_as_int(b)));           \
    }
