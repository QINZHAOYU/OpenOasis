...
// Disable the code for older versions of gcc that don't support many of the required
// avx512 instrinsics.
...
psqrt<Packet16f>(const Packet16f &_x)
{
    ...
    __mmask16 denormal_mask = _mm512_kand(
        _mm512_cmp_ps_mask(
            _x, pset1<Packet16f>((std::numeric_limits<float>::min)()), _CMP_LT_OQ),
...
psqrt<Packet8d>(const Packet8d &_x)
{
    ...
    __mmask16 denormal_mask = _mm512_kand(
        _mm512_cmp_pd_mask(
            _x, pset1<Packet8d>((std::numeric_limits<double>::min)()), _CMP_LT_OQ),
...
prsqrt<Packet16f>(const Packet16f &_x)
{
    ...
    Packet16f y_newton =
        pmul(y_approx, pmadd(y_approx, pmul(neg_half, y_approx), p16f_one_point_five));
...
#endif
    // Do one or two steps of Newton-Raphson's to improve the approximation, depending
    // on the starting accuracy (either 2^-14 or 2^-28, depending on whether AVX512ER is
    // available). The Newton-Raphson algorithm has quadratic convergence and roughly
    // doubles the number of correct digits for each step. This uses the formula y_{n+1}
    // = y_n * (1.5 - y_n * (0.5 * x) * y_n). It is essential to evaluate the inner term
    // like this because forming y_n^2 may over- or underflow.
    Packet8d y_newton =
        pmul(y_approx, pmadd(neg_half, pmul(y_approx, y_approx), p8d_one_point_five));
#if !defined(EIGEN_VECTORIZE_AVX512ER)
    y_newton =
        pmul(y_newton, pmadd(y_newton, pmul(neg_half, y_newton), p8d_one_point_five));
#endif
    // Select the result of the Newton-Raphson step for positive finite arguments.
    // For other arguments, choose the output of the intrinsic. This will
    // return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(0) = +inf.
