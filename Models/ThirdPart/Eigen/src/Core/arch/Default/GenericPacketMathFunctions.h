...
pfrexp_generic(const Packet &a, Packet &exponent)
{
    ...
    exponent_offset = pselect(
        is_denormal, padd(exponent_offset, normalization_offset), exponent_offset);
    ...
    // (technically the exponent is unspecified for inf/NaN, but GCC/Clang set it to
    // zero)
    ...
    const Packet m = pselect(
        is_zero_or_not_finite, a, por(pand(normalized_a, sign_mantissa_mask), half));
...
pldexp_generic(const Packet &a, const Packet &exponent)
{
    ...
    const Packet  max_exponent = pset1<Packet>(Scalar(
        (ScalarI(1) << int(ExponentBits)) + ScalarI(int(MantissaBits) - 1)));  // 278
    const PacketI bias =
        pset1<PacketI>((ScalarI(1) << (int(ExponentBits) - 1)) - ScalarI(1));  // 127
...
struct pldexp_fast_impl
{
    ...
    run(const Packet &a, const Packet &exponent)
    {
        ...
        const PacketI e = pcast<Packet, PacketI>(
            pmin(pmax(padd(exponent, bias), pzero(limit)), limit));  // exponent + 127
        // return a * (2^e)
...
inline float trig_reduce_huge(float xf, int *quadrant)
{
    ...
    *quadrant  = int(q);
    ...
    // since we have p=x/(pi/2) with high accuracy, we can more efficiently compute r
    // as:
    //   r = (p-q)*pi/2,
    // where the product can be be carried out with sufficient accuracy using double
    // precision.
    p -= q << 62;
...
// using "Extended precision modular arithmetic"
    ...
    x = pmadd(
        y,
        pset1<Packet>(-5.390302529957764765544681040410068817436695098876953125e-15f),
        x);
#else
    ...
    // The following set of coefficients maintain 1ULP up to 9.43 and 14.16 for sin and
    // cos respectively. and 2 ULP up to:
...
// x = pmadd(y, pset1<Packet>(-6.0771006282767103812147979624569416046142578125e-11),
// x);
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
    ...
    //    accuracy c = (A'*diag(w)*A)\(A'*diag(w)*(sin(x)-x)); # weighted LS, linear
    //    coeff forced to 1 printf('%.64f\n %.64f\n%.64f\n', c(3), c(2), c(1))
    //
...
psqrt_complex(const Packet &a)
{
    ...
    //
    // To derive the formula for the complex square roots, let's consider the equation
    // for a single complex square root of the number x + i*y. We want to find real
    ...
    // We don't care about the imaginary parts computed here. They will be overwritten
    // later.
    ...
    // prepare packet of (+∞,0*|y|) or (0*|y|,+∞), depending on the sign of the infinite
    // real part.
    Packet real_inf_result;
    real_inf_result.v =
        pmul(a_abs, pset1<Packet>(Scalar(RealScalar(1.0), RealScalar(0.0))).v);
    real_inf_result.v =
        pselect(negative_real_mask.v, pcplxflip(real_inf_result).v, real_inf_result.v);
    ...
    return pselect(
        is_imag_inf, imag_inf_result, pselect(is_real_inf, real_inf_result, result));
}
...
fast_twosum(const Packet &x, const Packet &y, Packet &s_hi, Packet &s_lo)
{
    s_hi           = padd(x, y);
...
template <typename Packet>
EIGEN_STRONG_INLINE void
twoprod(const Packet &x, const Packet &y, Packet &p_hi, Packet &p_lo)
{
    p_hi = pmul(x, y);
...
EIGEN_STRONG_INLINE void veltkamp_splitting(const Packet &x, Packet &x_hi, Packet &x_lo)
{
    ...
    const Scalar shift_scale =
        Scalar(uint64_t(1) << shift);  // Scalar constructor not necessarily constexpr.
...
// 3rd edition, Birkh\"auser, 2016.
template <typename Packet>
EIGEN_STRONG_INLINE void twosum(
    const Packet &x_hi, const Packet &x_lo, const Packet &y_hi, const Packet &y_lo,
...
template <typename Packet>
EIGEN_STRONG_INLINE void fast_twosum(
    const Packet &x_hi, const Packet &x_lo, const Packet &y_hi, const Packet &y_lo,
    Packet &s_hi, Packet &s_lo)
{
    Packet r_hi, r_lo;
...
template <typename Packet>
EIGEN_STRONG_INLINE void fast_twosum(
    const Packet &x, const Packet &y_hi, const Packet &y_lo, Packet &s_hi, Packet &s_lo)
{
    Packet r_hi, r_lo;
...
// 3rd edition, Birkh\"auser, 2016.
template <typename Packet>
EIGEN_STRONG_INLINE void twoprod(
    const Packet &x_hi, const Packet &x_lo, const Packet &y, Packet &p_hi, Packet &p_lo)
{
    Packet c_hi, c_lo1;
...
struct accurate_log2
{
    template <typename Packet>
    EIGEN_STRONG_INLINE void
    operator()(const Packet &x, Packet &log2_x_hi, Packet &log2_x_lo)
    {
        log2_x_hi = plog2(x);
        log2_x_lo = pzero(x);
    }
...
struct accurate_log2<float>
{
    template <typename Packet>
    EIGEN_STRONG_INLINE void
    operator()(const Packet &z, Packet &log2_x_hi, Packet &log2_x_lo)
    {
...
struct fast_accurate_exp2<float>
{
    ...
    EIGEN_STRONG_INLINE Packet operator()(const Packet &x)
    {
        ...
        // extra precision using double word arithmetic. C is an extra precise constant
