...
#define EIGEN_MAKE_CONJ_HELPER_CPLX_REAL(PACKET_CPLX, PACKET_REAL)                     \
    ...
    {                                                                                  \
        ...
        pmadd(const PACKET_REAL &x, const PACKET_CPLX &y, const PACKET_CPLX &c) const  \
        {                                                                              \
            return padd(c, this->pmul(x, y));                                          \
        }                                                                              \
        EIGEN_STRONG_INLINE PACKET_CPLX                                                \
        pmul(const PACKET_REAL &x, const PACKET_CPLX &y) const                         \
        {                                                                              \
    ...
    {                                                                                  \
        EIGEN_STRONG_INLINE PACKET_CPLX                                                \
        pmadd(const PACKET_CPLX &x, const PACKET_REAL &y, const PACKET_CPLX &c) const  \
        {                                                                              \
            return padd(c, this->pmul(x, y));                                          \
        }                                                                              \
        EIGEN_STRONG_INLINE PACKET_CPLX                                                \
        pmul(const PACKET_CPLX &x, const PACKET_REAL &y) const                         \
        {                                                                              \
...
struct conj_helper<Packet, Packet, ConjLhs, ConjRhs>
{
    ...
    pmadd(const Packet &x, const Packet &y, const Packet &c) const
    {
        return Eigen::internal::pmadd(
            conj_if<ConjLhs>().pconj(x), conj_if<ConjRhs>().pconj(y), c);
    }


    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE Packet
    pmul(const Packet &x, const Packet &y) const
    {
        return Eigen::internal::pmul(
            conj_if<ConjLhs>().pconj(x), conj_if<ConjRhs>().pconj(y));
    }
};
