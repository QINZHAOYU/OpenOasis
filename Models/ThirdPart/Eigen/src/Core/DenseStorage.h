...
// if EIGEN_STACK_ALLOCATION_LIMIT is defined to 0, then no limit
    ...
    EIGEN_STATIC_ASSERT(
        Size * sizeof(T) <= EIGEN_STACK_ALLOCATION_LIMIT,
        OBJECT_ALLOCATED_ON_STACK_IS_TOO_BIG);
#endif
}

/** \internal
 * Static array. If the MatrixOrArrayOptions require auto-alignment, the array will be
 * automatically aligned: to 16 bytes boundary if the total size is a multiple of 16
 * bytes.
 */
template <
    typename T, int Size, int MatrixOrArrayOptions,
...
// GCC 4.7 is too aggressive in its optimizations and remove the alignment test based on
// the fact the array is declared to be aligned. See this bug report:
// http://gcc.gnu.org/bugzilla/show_bug.cgi?id=53900 Hiding the origin of the array
// pointer behind a function argument seems to do the trick even if the function is
// inlined:
...
public:
    ...
    EIGEN_DEVICE_FUNC void swap(DenseStorage &other)
    {
        internal::plain_array_helper::swap(
            m_data, m_rows * m_cols, other.m_data, other.m_rows * other.m_cols);
...
public:
    ...
    {
        internal::plain_array_helper::swap(
            m_data, m_rows * _Cols, other.m_data, other.m_rows * _Cols);
        numext::swap(m_rows, other.m_rows);
    }
...
public:
    ...
    {
        internal::plain_array_helper::swap(
            m_data, _Rows * m_cols, other.m_data, _Rows * other.m_cols);
        numext::swap(m_cols, other.m_cols);
    }
...
public:
        ...
        m_rows(other.m_rows), m_cols(other.m_cols)
    {
        ...
        internal::smart_copy(
            other.m_data, other.m_data + other.m_rows * other.m_cols, m_data);
    }
    EIGEN_DEVICE_FUNC DenseStorage &operator=(const DenseStorage &other)
    {
        if (this != &other)
    ...
    {
        m_data = internal::
            conditional_aligned_realloc_new_auto<T, (_Options & DontAlign) == 0>(
    ...
    EIGEN_DEVICE_FUNC void resize(Index size, Index rows, Index cols)
    {
        if (size != m_rows * m_cols)
        {
            ...
            if (size > 0)  // >0 and not simply !=0 to let the compiler knows that size
                           // cannot be negative
                m_data = internal::
                    conditional_aligned_new_auto<T, (_Options & DontAlign) == 0>(size);
            else
                m_data = 0;
            EIGEN_INTERNAL_DENSE_STORAGE_CTOR_PLUGIN({})
        }
