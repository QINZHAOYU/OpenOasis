...
template <
    typename _LhsScalar, typename _RhsScalar, bool _ConjLhs = false,
...
/** \internal */
inline void manage_caching_sizes(
    Action action, std::ptrdiff_t *l1, std::ptrdiff_t *l2, std::ptrdiff_t *l3)
{
    static CacheSizes m_cacheSizes;

    if (action == SetAction)
    {
        // set the cpu cache size and cache all block sizes from a global cache size in
        // byte
 ...
 * RhsScalar, this function computes the blocking size parameters along the respective
 * dimensions for matrix products and related algorithms. The blocking sizes depends on
 * various parameters:
...
void evaluateProductBlockingSizesHeuristic(
    Index &k, Index &m, Index &n, Index num_threads = 1)
{
    typedef gebp_traits<LhsScalar, RhsScalar> Traits;

    // Explanations:
    // Let's recall that the product algorithms form mc x kc vertical panels A' on the
    // lhs and kc x nc blocks B' on the rhs. B' has to fit into L2/L3 cache. Moreover,
    // A' is processed per mr x kc horizontal small panels where mr is the blocking size
    // along the m dimension at the register level. This small horizontal panel has to
    ...
    // For instance, with the 3pX4 kernel and double, the size of the lhs+rhs panels
    // are:
    ...
    if (num_threads > 1)
    {
        ...
        {
            n = (numext::mini<
                 Index>)(n, (n_per_thread + nr - 1) - ((n_per_thread + nr - 1) % nr));
        }
        ...
        if (l3 > l2)
        {
            ...
            {
                m = (numext::mini<
                     Index>)(m, (m_per_thread + mr - 1) - ((m_per_thread + mr - 1) % mr));
            }
        }
    }
    else
    {
        ...
        // consuming for small problems. Perhaps it would make more sense to consider
        // k*n*m?? Note that for very tiny problem, this function should be bypassed
        // anyway because we use the coefficient-based implementation for them.
        ...
        // panel of size mr x kc of the lhs plus a vertical panel of kc x nr of the rhs
        // both fits within L1 cache. We also include a register-level block of the
        // result (mx x nr). (In an ideal world only the lhs panel would stay in L1)
        // Moreover, kc has to be a multiple of 8 to be compatible with loop peeling,
        // leading to a maximum blocking size of:
        const Index max_kc =
        ...
        if (k > max_kc)
        {
            ...
            eigen_internal_assert(
                ((old_k / k) == (old_k / max_kc))
                && "the number of sweeps has to remain the same");
        }

// ---- 2nd level of blocking on max(L2,L3), yields nc ----

// TODO find a reliable way to get the actual amount of cache per core to use for 2nd
// level blocking, that is:
//      actual_l2 = max(l2, l3/nb_core_sharing_l3)
// The number below is quite conservative: it is better to underestimate the cache size
// rather than overestimating it) For instance, it corresponds to 6MB of L3 shared among
// 4 cores.
#ifdef EIGEN_DEBUG_SMALL_PRODUCT_BLOCKS
        ...
        // Here, nc is chosen such that a block of kc x nc of the rhs fit within half of
        // L2. The second half is implicitly reserved to access the result and lhs
        // coefficients. When k<max_kc, then nc can arbitrarily growth. In practice, it
        // seems to be fruitful to limit this growth: we bound nc to growth by a factor
        // x1.5. However, if the entire lhs block fit within L1, then we are not going
        // to block on the rows at all, and it becomes fruitful to keep the packed rhs
        // blocks in L1 if there is enough remaining space.
        Index       max_nc;
        ...
        Index nc = numext::mini<Index>(actual_l2 / (2 * k * sizeof(RhsScalar)), max_nc)
                   & (~(Traits::nr - 1));
        if (n > nc)
        {
        ...
        else if (old_k == k)
        {
            ...
            // In this case, let's perform a blocking over the rows such that the packed
            // lhs data is kept in cache L1/L2
            // TODO: part of this blocking strategy is now implemented within the kernel
            // itself, so the L1-based heuristic here should be obsolete.
            ...
            Index mc =
                (numext::mini<Index>)(actual_lm / (3 * k * sizeof(LhsScalar)), max_mc);
            if (mc > Traits::mr)
                mc -= mc % Traits::mr;
            else if (mc == 0)
                return;
 ...
 *
 * \param[in,out] k Input: the third dimension of the product. Output: the blocking size
 * along the same dimension. \param[in,out] m Input: the number of rows of the left hand
 * side. Output: the blocking size along the same dimension. \param[in,out] n Input: the
 * number of columns of the right hand side. Output: the blocking size along the same
 * dimension.
 *
 * Given a m x k times k x n matrix product of scalar types \c LhsScalar and \c
 * RhsScalar, this function computes the blocking size parameters along the respective
 * dimensions for matrix products and related algorithms.
 *
...
void computeProductBlockingSizes(Index &k, Index &m, Index &n, Index num_threads = 1)
{
    ...
    {
        evaluateProductBlockingSizesHeuristic<LhsScalar, RhsScalar, KcFactor, Index>(
            k, m, n, num_threads);
    }
}
...
public:
    typedef typename conditional<remaining_registers >= 4, RhsPacketx4, RhsPacket>::type
        type;
};
...
#define PACKET_DECL_COND_PREFIX(prefix, name, packet_size)                             \
    typedef typename packet_conditional<                                               \
        ...
        typename unpacket_traits<typename packet_traits<name##Scalar>::half>::half>::  \
        type prefix##name##Packet
 ...
 *
 *  cd*cd : unpack rhs to (b_r,b_r), (b_i,b_i), mul to get (a_r b_r,a_i b_r) (a_r
 * b_i,a_i b_i), storing each res packet into two packets (2x2), at the end combine
 * them: swap the second and addsub them cf*cf : same but with 2x4 blocks cplx*real :
 * unpack rhs to constant packets, ... real*cplx : load lhs as (a0,a0,a1,a1), and mul as
 * usual
 */
template <
    typename _LhsScalar, typename _RhsScalar, bool _ConjLhs, bool _ConjRhs, int Arch,
    int _PacketSize>
class gebp_traits
{
public:
    ...
    && ((!EIGEN_COMP_MSVC) || (EIGEN_COMP_MSVC >= 1914))
        ...
        // triggers non-implemented paths in syrk. Bug 1515: MSVC prior to v19.14 yields
    ...
    template <
        typename LhsPacketType, typename RhsPacketType, typename AccPacketType,
        typename LaneIdType>
    EIGEN_STRONG_INLINE void madd(
    ...
    EIGEN_STRONG_INLINE void madd(
        const LhsPacketType &a, const RhsPacketx4 &b, AccPacketType &c, RhsPacket &tmp,
...
public:
    ...
    EIGEN_STRONG_INLINE void
    loadRhsQuad_impl(const RhsScalar *b, RhsPacket &dest, const true_type &) const
    {
    ...
    EIGEN_STRONG_INLINE void
    loadRhsQuad_impl(const RhsScalar *b, RhsPacket &dest, const false_type &) const
    {
    ...
    EIGEN_STRONG_INLINE void madd_impl(
        const LhsScalar &a, const RhsScalar &b, ResScalar &c, RhsScalar & /*tmp*/,
    ...
    EIGEN_STRONG_INLINE void
    acc(const AccPacketType &c, const ResPacketType &alpha, ResPacketType &r) const
    {
    ...
    typename enable_if<unpacket_traits<RealPacket>::size == 16>::type * = 0)
{
    ...
    RealScalar                               r[4] = {
        numext::real(b[0]), numext::real(b[0]), numext::real(b[1]), numext::real(b[1])};
    RealScalar i[4] = {
        numext::imag(b[0]), numext::imag(b[0]), numext::imag(b[1]), numext::imag(b[1])};
...
// template<typename Packet>
// DoublePacket<Packet> pmadd(const DoublePacket<Packet> &a, const DoublePacket<Packet>
// &b)
// {
//   DoublePacket<Packet> res;
//   res.first  = padd(a.first, b.first);
...
class gebp_traits<
    std::complex<RealScalar>, std::complex<RealScalar>, _ConjLhs, _ConjRhs, Arch,
    _PacketSize>
{
public:
    ...
    typedef
        typename conditional<Vectorizable, DoublePacketType, Scalar>::type RhsPacket;
    typedef typename conditional<Vectorizable, ScalarPacket, Scalar>::type ResPacket;
    typedef
        typename conditional<Vectorizable, DoublePacketType, Scalar>::type AccPacket;
    ...
    EIGEN_STRONG_INLINE void
    loadRhs(const RhsScalar *b, DoublePacket<RealPacketType> &dest) const
    {
    ...
    template <typename RealPacketType>
    EIGEN_STRONG_INLINE void
    updateRhs(const RhsScalar *b, DoublePacket<RealPacketType> &dest) const
    {
        loadRhs(b, dest);
    }
    ...
    EIGEN_STRONG_INLINE void
    loadRhsQuad(const RhsScalar *b, DoublePacketType &dest) const
    {
    ...
    template <
        typename LhsPacketType, typename RhsPacketType, typename ResPacketType,
        typename TmpType, typename LaneIdType>
    EIGEN_STRONG_INLINE
        typename enable_if<!is_same<RhsPacketType, RhsPacketx4>::value>::type
        madd(
            const LhsPacketType &a, const RhsPacketType &b,
            DoublePacket<ResPacketType> &c, TmpType & /*tmp*/, const LaneIdType &) const
    {
    ...
    EIGEN_STRONG_INLINE void madd(
        const LhsPacket &a, const RhsPacket &b, ResPacket &c, RhsPacket & /*tmp*/,
    ...
    EIGEN_STRONG_INLINE void
    acc(const DoublePacket<RealPacketType> &c, const ResPacketType &alpha,
...
class gebp_traits<
    RealScalar, std::complex<RealScalar>, false, _ConjRhs, Arch, _PacketSize>
{
public:
    typedef std::complex<RealScalar> Scalar;
 ...
 */
template <
    typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper, int mr,
    int nr, bool ConjugateLhs, bool ConjugateRhs>
struct gebp_kernel
{
    typedef gebp_traits<
        LhsScalar, RhsScalar, ConjugateLhs, ConjugateRhs, Architecture::Target>
        Traits;
    typedef gebp_traits<
        LhsScalar, RhsScalar, ConjugateLhs, ConjugateRhs, Architecture::Target,
        GEBPPacketHalf>
    ...
    typedef gebp_traits<
        RhsScalar, LhsScalar, ConjugateRhs, ConjugateLhs, Architecture::Target>
        SwappedTraits;
    ...
    void operator()(
        ...
        Index rows, Index depth, Index cols, ResScalar alpha, Index strideA = -1,
...
template <
    typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper, int mr,
    int nr, bool ConjugateLhs, bool ConjugateRhs,
    int SwappedLhsProgress = gebp_traits<
        RhsScalar, LhsScalar, ConjugateRhs, ConjugateLhs,
...
struct last_row_process_16_packets
{
    ...
    EIGEN_STRONG_INLINE void operator()(
        const DataMapper &res, SwappedTraits &straits, const LhsScalar *blA,
...
template <
    ...
    int nr, bool ConjugateLhs, bool ConjugateRhs>
struct last_row_process_16_packets<
    LhsScalar, RhsScalar, Index, DataMapper, mr, nr, ConjugateLhs, ConjugateRhs, 16>
{
    typedef gebp_traits<
        LhsScalar, RhsScalar, ConjugateLhs, ConjugateRhs, Architecture::Target>
        Traits;
...
template <
    int nr, Index LhsProgress, Index RhsProgress, typename LhsScalar,
    typename RhsScalar, typename ResScalar, typename AccPacket, typename LhsPacket,
    typename RhsPacket, typename ResPacket, typename GEBPTraits, typename LinearMapper,
    typename DataMapper>
struct lhs_process_one_packet
{
    ...
    EIGEN_STRONG_INLINE void operator()(
        ...
        Index offsetA, Index offsetB, int prefetch_res_offset, Index peeled_kc,
        Index pk, Index cols, Index depth, Index packet_cols4)
    {
        GEBPTraits traits;
        ...
        for (Index i = peelStart; i < peelEnd; i += LhsProgress)
        {
            ...
            for (Index j2 = 0; j2 < packet_cols4; j2 += nr)
            {
                ...
                for (Index k = 0; k < peeled_kc; k += pk)
                {
                    ...
                    peeled_kc_onestep(
                        0, blA, blB, traits, &A0, &rhs_panel, &T0, &C0, &C1, &C2, &C3);
                    peeled_kc_onestep(
                        1, blA, blB, traits, &A1, &rhs_panel, &T0, &D0, &D1, &D2, &D3);
                    peeled_kc_onestep(
                        2, blA, blB, traits, &A0, &rhs_panel, &T0, &C0, &C1, &C2, &C3);
                    peeled_kc_onestep(
                        3, blA, blB, traits, &A1, &rhs_panel, &T0, &D0, &D1, &D2, &D3);
                    internal::prefetch(blB + (48 + 16));
                    peeled_kc_onestep(
                        4, blA, blB, traits, &A0, &rhs_panel, &T0, &C0, &C1, &C2, &C3);
                    peeled_kc_onestep(
                        5, blA, blB, traits, &A1, &rhs_panel, &T0, &D0, &D1, &D2, &D3);
                    peeled_kc_onestep(
                        6, blA, blB, traits, &A0, &rhs_panel, &T0, &C0, &C1, &C2, &C3);
                    peeled_kc_onestep(
                        7, blA, blB, traits, &A1, &rhs_panel, &T0, &D0, &D1, &D2, &D3);
...
struct lhs_process_fraction_of_packet
    : lhs_process_one_packet<
          nr, LhsProgress, RhsProgress, LhsScalar, RhsScalar, ResScalar, AccPacket,
          LhsPacket, RhsPacket, ResPacket, GEBPTraits, LinearMapper, DataMapper>
{
    EIGEN_STRONG_INLINE void peeled_kc_onestep(
        Index K, const LhsScalar *blA, const RhsScalar *blB, GEBPTraits traits,
...
template <
    typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper, int mr,
    int nr, bool ConjugateLhs, bool ConjugateRhs>
EIGEN_DONT_INLINE void gebp_kernel<
    LhsScalar, RhsScalar, Index, DataMapper, mr, nr, ConjugateLhs, ConjugateRhs>::
operator()(
    const DataMapper &res, const LhsScalar *blockA, const RhsScalar *blockB, Index rows,
    Index depth, Index cols, ResScalar alpha, Index strideA, Index strideB,
    Index offsetA, Index offsetB)
{
    ...
    const Index peeled_mc_half =
        mr >= LhsProgressHalf ?
            peeled_mc1 + ((rows - peeled_mc1) / (LhsProgressHalf)) * (LhsProgressHalf) :
            0;
    const Index peeled_mc_quarter =
        mr >= LhsProgressQuarter ?
            peeled_mc_half
                + ((rows - peeled_mc_half) / (LhsProgressQuarter))
                      * (LhsProgressQuarter) :
            0;
    enum
    {
        pk = 8
    };  // NOTE Such a large peeling factor is important for large matrices (~ +5% when
        // >1000 on Haswell)
    ...
    {
        // Here, the general idea is to loop on each largest micro horizontal panel of
        // the lhs (3*Traits::LhsProgress x depth) and on each largest micro vertical
        // panel of the rhs (depth * nr). Blocking sizes, i.e., 'depth' has been
        // computed so that the micro horizontal panel of the lhs fit in L1. However, if
        // depth is too small, we can extend the number of rows of these horizontal
        // panels. This actual number of rows is computed as follow:
        const Index l1 = defaultL1CacheSize;  // in Bytes, TODO, l1 should be passed to
                                              // this function.
        // The max(1, ...) here is needed because we may be using blocking params larger
        // than what our known l1 cache size suggests we should be using: either because
        // our known l1 cache size is inaccurate (e.g. on Android, we can only guess),
            ...
            for (Index i = peeled_mc_quarter; i < rows; i += 1)
            {
                        ...
                        || SResPacketQuarterSize == nr))
                {
                    ...
                    for (; k < endk4; k += 4 * spk)
                    {
                        ...
                        straits.loadLhsUnaligned(
                            blB + 0 * SwappedTraits::LhsProgress, A0);
                        straits.loadLhsUnaligned(
                            blB + 1 * SwappedTraits::LhsProgress, A1);
                        ...
                        straits.loadLhsUnaligned(
                            blB + 2 * SwappedTraits::LhsProgress, A0);
                        straits.loadLhsUnaligned(
                            blB + 3 * SwappedTraits::LhsProgress, A1);
                    ...
                    if (SwappedTraits::LhsProgress == 8)
                    {
                        ...
                        SResPacketHalf R =
                            res.template gatherPacket<SResPacketHalf>(i, j2);
                        SResPacketHalf alphav = pset1<SResPacketHalf>(alpha);

                        if (depth - endk > 0)
                        {
                            // We have to handle the last row of the rhs which
                            // corresponds to a half-packet
                            SLhsPacketHalf a0;
                            SRhsPacketHalf b0;
                            straits.loadLhsUnaligned(blB, a0);
                            straits.loadRhs(blA, b0);
...
//  36 36 38 39 ...
template <
    typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
    typename Packet, bool Conjugate, bool PanelMode>
struct gemm_pack_lhs<
    Scalar, Index, DataMapper, Pack1, Pack2, Packet, ColMajor, Conjugate, PanelMode>
{
    typedef typename DataMapper::LinearMapper LinearMapper;
    EIGEN_DONT_INLINE void                    operator()(
        Scalar *blockA, const DataMapper &lhs, Index depth, Index rows,
...
template <
    typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
    typename Packet, bool Conjugate, bool PanelMode>
EIGEN_DONT_INLINE void gemm_pack_lhs<
    Scalar, Index, DataMapper, Pack1, Pack2, Packet, ColMajor, Conjugate, PanelMode>::
operator()(
    ...
    Index offset)
{
    ...
    const Index peeled_mc1 =
        Pack1 >= 1 * PacketSize ?
            peeled_mc2 + ((rows - peeled_mc2) / (1 * PacketSize)) * (1 * PacketSize) :
            0;
    const Index peeled_mc_half =
        Pack1 >= HalfPacketSize ?
            peeled_mc1 + ((rows - peeled_mc1) / (HalfPacketSize)) * (HalfPacketSize) :
            0;
    const Index peeled_mc_quarter =
    ...
    if (HasQuarter && Pack1 >= QuarterPacketSize)
    {
        for (; i < peeled_mc_quarter; i += QuarterPacketSize)
        {
            ...
            {
                QuarterPacket A;
                A = lhs.template loadPacket<QuarterPacket>(
...
template <
    ...
    typename Packet, bool Conjugate, bool PanelMode>
struct gemm_pack_lhs<
    Scalar, Index, DataMapper, Pack1, Pack2, Packet, RowMajor, Conjugate, PanelMode>
{
    typedef typename DataMapper::LinearMapper LinearMapper;
    EIGEN_DONT_INLINE void                    operator()(
        Scalar *blockA, const DataMapper &lhs, Index depth, Index rows,
                        ...
                        else if (HasQuarter && psize == QuarterPacketSize)
                        {
                            ...
                            for (int p = 0; p < psize; ++p)
                                kernel_quarter.packet[p] =
                                    lhs.template loadPacket<QuarterPacket>(
                                        i + p + m, k);
...
//  .  .  .  .    .  .  .  .    .  .
template <
    typename Scalar, typename Index, typename DataMapper, int nr, bool Conjugate,
    bool PanelMode>
struct gemm_pack_rhs<Scalar, Index, DataMapper, nr, ColMajor, Conjugate, PanelMode>
{
    ...
    EIGEN_DONT_INLINE void operator()(
        Scalar *blockB, const DataMapper &rhs, Index depth, Index cols,
        Index stride = 0, Index offset = 0);
};
...
operator()(
    Scalar *blockB, const DataMapper &rhs, Index depth, Index cols, Index stride,
    Index offset)
{
        ...
        for (Index j2 = packet_cols8; j2 < packet_cols4; j2 += 4)
        {
                ...
                == 0)  // TODO enable vectorized transposition for PacketSize==2 ??
            {
                for (; k < peeled_k; k += PacketSize)
                {
                    ...
                    ptranspose(kernel);
                    pstoreu(
                        blockB + count + 0 * PacketSize, cj.pconj(kernel.packet[0]));
                    pstoreu(
                        blockB + count + 1 * PacketSize,
                        cj.pconj(kernel.packet[1 % PacketSize]));
                    pstoreu(
                        blockB + count + 2 * PacketSize,
                        cj.pconj(kernel.packet[2 % PacketSize]));
                    pstoreu(
                        blockB + count + 3 * PacketSize,
...
/** \returns the currently set level 1 cpu cache size (in bytes) used to estimate the
 * ideal blocking size parameters. \sa setCpuCacheSize */
...
/** \returns the currently set level 2 cpu cache size (in bytes) used to estimate the
 * ideal blocking size parameters. \sa setCpuCacheSize */
...
/** \returns the currently set level 3 cpu cache size (in bytes) used to estimate the
ideal blocking size paramete\ rs.
* \sa setCpuCacheSize */
