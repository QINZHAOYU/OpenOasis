 ...
 *  |real |cplx |real | alpha is converted to a cplx when calling the run function, no
 * vectorization |cplx |real |cplx | invalid, the caller has to do tmp: = A * B; C +=
 * alpha*tmp |cplx |real |real | optimal case, vectorization possible via real-cplx mul
 *
 * The same reasoning apply for the transposed case.
 */
template <
    typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs,
...
EIGEN_DEVICE_FUNC EIGEN_DONT_INLINE void general_matrix_vector_product<
    Index, LhsScalar, LhsMapper, ColMajor, ConjugateLhs, RhsScalar, RhsMapper,
    ConjugateRhs, Version>::
    run(Index rows, Index cols, const LhsMapper &alhs, const RhsMapper &rhs,
        ResScalar *res, Index resIncr, RhsScalar alpha)
{
    EIGEN_UNUSED_VARIABLE(resIncr);
    eigen_internal_assert(resIncr == 1);

    // The following copy tells the compiler that lhs's attributes are not modified
    // outside this function This helps GCC to generate propoer code.
    LhsMapper lhs(alhs);
    ...
    for (Index j2 = 0; j2 < cols; j2 += block_cols)
    {
        ...
        for (; i < n8; i += ResPacketSize * 8)
        {
            ...
            for (Index j = j2; j < jend; j += 1)
            {
                ...
                c0           = pcj.pmadd(
                    lhs.template load<LhsPacket, LhsAlignment>(
                        i + LhsPacketSize * 0, j),
                    b0,
                    c0);
                c1 = pcj.pmadd(
        ...
        if (i < n1)
        {
            ...
            for (Index j = j2; j < jend; j += 1)
            {
                RhsPacket b0 = pset1<RhsPacket>(rhs(j, 0));
                c0           = pcj.pmadd(
                    lhs.template load<LhsPacket, LhsAlignment>(i + 0, j), b0, c0);
            }
            pstoreu(
                res + i + ResPacketSize * 0,
                pmadd(c0, palpha, ploadu<ResPacket>(res + i + ResPacketSize * 0)));
        ...
        if (HasQuarter && i < n_quarter)
        {
            ...
            for (Index j = j2; j < jend; j += 1)
            {
                ...
                c0                  = pcj_quarter.pmadd(
                    lhs.template load<LhsPacketQuarter, LhsAlignment>(i + 0, j),
                    b0,
                    c0);
            }
            pstoreu(
        ...
        ResScalar *res, Index resIncr, ResScalar alpha)
{
    ...
    // TODO: fine tune the following heuristic. The rationale is that if the matrix is
    // very large,
    ...
    for (; i < n8; i += 8)
    {
        ...
        for (; j + LhsPacketSize <= cols; j += LhsPacketSize)
        {
            ...
            c0 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 0, j), b0, c0);
            c1 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 1, j), b0, c1);
            c2 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 2, j), b0, c2);
            c3 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 3, j), b0, c3);
            c4 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 4, j), b0, c4);
            c5 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 5, j), b0, c5);
            c6 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 6, j), b0, c6);
            c7 =
                pcj.pmadd(lhs.template load<LhsPacket, LhsAlignment>(i + 7, j), b0, c7);
        }
