...
// When constness must affect traits, it has to be constness on template parameters on
// which T itself depends. For example, traits<Map<const T> > != traits<Map<T> >, but
...
template <
    typename _Scalar, int SizeAtCompileTime,
...
class Diagonal;
template <
    int SizeAtCompileTime, int MaxSizeAtCompileTime = SizeAtCompileTime,
...
class OuterStride;
template <
    typename MatrixType, int MapOptions = Unaligned, typename StrideType = Stride<0, 0>>
class Map;
template <typename Derived>
class RefBase;
template <
    ...
    typename StrideType = typename internal::conditional<
        PlainObjectType::IsVectorAtCompileTime, InnerStride<1>, OuterStride<>>::type>
class Ref;
...
{
template <
    typename _Scalar, int Rows = Dynamic, int Cols = Dynamic, int Supers = Dynamic,
    int Subs = Dynamic, int Options = 0>
class BandMatrix;
}
...
template <
    typename LhsScalar, typename RhsScalar = LhsScalar,
