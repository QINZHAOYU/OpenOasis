 ...
 *
 *  - in EIGEN_STATIC_ASSERT(CONDITION,MSG) the parameter CONDITION must be a compile
 * time boolean expression, and MSG an enum listed in struct
...
#define EIGEN_STATIC_ASSERT_VECTOR_ONLY(TYPE)                                          \
    EIGEN_STATIC_ASSERT(                                                               \
        TYPE::IsVectorAtCompileTime, YOU_TRIED_CALLING_A_VECTOR_METHOD_ON_A_MATRIX)
...
// static assertion failing if the two vector expression types are not compatible (same
...
#define EIGEN_STATIC_ASSERT_NON_INTEGER(TYPE)                                          \
    EIGEN_STATIC_ASSERT(                                                               \
        !Eigen::NumTraits<TYPE>::IsInteger,                                            \
        THIS_FUNCTION_IS_NOT_FOR_INTEGER_NUMERIC_TYPES)


// static assertion failing if it is guaranteed at compile-time that the two matrix
// expression types have different sizes
...
#define EIGEN_STATIC_ASSERT_SIZE_1x1(TYPE)                                             \
    EIGEN_STATIC_ASSERT(                                                               \
        (TYPE::RowsAtCompileTime == 1 || TYPE::RowsAtCompileTime == Eigen::Dynamic)    \
...
#define EIGEN_STATIC_ASSERT_LVALUE(Derived)                                            \
    EIGEN_STATIC_ASSERT(                                                               \
        Eigen::internal::is_lvalue<Derived>::value,                                    \
