 ...
 * Such optimization are enabled by default, and set EIGEN_FAST_MATH to 0 to disable
 * them. They currently include:
...
/// \internal EIGEN_COMP_CLANG set to major+minor version (e.g., 307 for clang 3.7) if
...
/// \internal EIGEN_COMP_ICC set to __INTEL_COMPILER if the compiler is Intel compiler,
/// 0 otherwise
...
/// \internal EIGEN_COMP_MSVC set to _MSC_VER if the compiler is Microsoft Visual C++, 0
/// otherwise.
#if defined(_MSC_VER)
...
/// \internal EIGEN_COMP_MSVC_STRICT set to 1 if the compiler is really Microsoft Visual
/// C++ and not ,e.g., ICC or clang-cl
...
/// \internal EIGEN_COMP_PGI set to PGI version if the compiler is Portland Group
/// Compiler
#if defined(__PGI)
...
#if EIGEN_COMP_GNUC                                                                    \
    && !(                                                                              \
        EIGEN_COMP_CLANG || EIGEN_COMP_ICC || EIGEN_COMP_MINGW || EIGEN_COMP_PGI       \
        || EIGEN_COMP_IBM || EIGEN_COMP_ARM || EIGEN_COMP_EMSCRIPTEN)
...
// note: ANDROID is defined when using ndk_build, __ANDROID__ is defined when using a
...
/// \internal EIGEN_OS_GNULINUX set to 1 if the OS is GNU Linux and not Linux-based OS
/// (e.g., not android)
...
/// \internal EIGEN_OS_WIN_STRICT set to 1 if the OS is really Windows and not some
/// variants
...
#define EIGEN_GPUCC
//
// EIGEN_HIPCC implies the HIP compiler and is used to tweak Eigen code for use in HIP
// kernels EIGEN_CUDACC implies the CUDA compiler and is used to tweak Eigen code for
// use in CUDA kernels
//
// In most cases the same tweaks are required to the Eigen code to enable in both the
// HIP and CUDA kernels. For those cases, the corresponding code should be guarded with
...
//
// Code that needs to enabled only during the either the "host" or "device" compilation
// phase needs to be guarded with a macro that indicates the current compilation phase
//
...
// compiler-defined macro. In most cases we want to check if both macros are defined
...
#define EIGEN_HAS_STD_INVOKE_RESULT 0
    ...
    && (__has_feature(cxx_alignas) || EIGEN_HAS_CXX14 || (EIGEN_COMP_MSVC >= 1800)     \
        || (EIGEN_GNUC_AT_LEAST(4, 8)) || (EIGEN_COMP_CLANG >= 305)                    \
        || (EIGEN_COMP_ICC >= 1500) || (EIGEN_COMP_PGI >= 1500)                        \
...
// ^^ Disable the use of variadic templates when compiling with versions of nvcc older
// than 8.0 on ARM devices:
...
#if defined(EIGEN_CUDACC)
// Const expressions are supported provided that c++11 is enabled and we're using either
// clang or nvcc 7.5 or above
#if EIGEN_MAX_CPP_VER >= 14                                                            \
    && (EIGEN_COMP_CXXVER >= 11 && (EIGEN_COMP_CLANG || EIGEN_COMP_NVCC >= 70500))
...
// Let's be conservative and enable the default C++11 implementation only if we are sure
// it exists
...
// While available already with c++11, this is useful mostly starting with c++14 and
...
// but it still doesn't use GCC's always_inline. This is useful in (common) situations
// where MSVC needs forceinline but GCC is still doing fine with just inline.
...
// EIGEN_ALWAYS_INLINE is the stronget, it has the effect of making the function inline
// and adding every possible attribute to maximize inlining. This should only be used
// when really necessary: in particular, it uses __attribute__((always_inline)) on GCC,
...
//  - static is not very good because it prevents definitions from different object
//  files to be merged.
//           So static causes the resulting linked executable to be bloated with
//           multiple copies of the same function.
//  - inline is not perfect either as it unwantedly hints the compiler toward inlining
//  the function.
...
// eigen_plain_assert is where we implement the workaround for the assert() bug in GCC
// <= 4.3, see bug 89
...
namespace internal
{
// trivial function copying a bool. Must be EIGEN_DONT_INLINE, so we implement it after
// including Eigen headers. see bug 89.
namespace
{
...
inline void
assert_fail(const char *condition, const char *function, const char *file, int line)
{
...
#define eigen_plain_assert(x)                                                          \
    ...
    {                                                                                  \
        if (!Eigen::internal::copy_bool(x))                                            \
            Eigen::internal::assert_fail(                                              \
                EIGEN_MAKESTRING(x), __PRETTY_FUNCTION__, __FILE__, __LINE__);         \
    } while (false)
#endif
#endif
...
// needed to define it here as escaping characters in CMake add_definition's argument
...
// For older MSVC versions, as well as 1900 && CUDA 8, using the base operator is
// necessary,
 ...
 * This is necessary, because the implicitly defined assignment operator gets deleted
 * when a custom operator= is defined. With C++11 or later this also default-implements
 ...
 * For empty helper classes this should usually be protected, to avoid accidentally
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived)                                        \
    ...
    typedef typename Eigen::NumTraits<Scalar>::Real                                    \
        RealScalar; /*!< \brief The underlying numeric type for composed scalar types. \
                       \details In cases where Scalar is e.g. std::complex<T>, T were  \
                       corresponding to RealScalar. */                                 \
    typedef typename Base::CoeffReturnType                                             \
        CoeffReturnType; /*!< \brief The return type for coefficient access. \details  \
                            Depending on whether the object allows direct coefficient  \
                            access (e.g. for a MatrixXd), this type is either 'const   \
                            Scalar&' or simply 'Scalar' for objects that do not allow  \
...
// absolute priority, followed by 1, followed by Dynamic, followed by other finite
...
// EIGEN_SIZE_MIN_PREFER_FIXED is a variant of EIGEN_SIZE_MIN_PREFER_DYNAMIC comparing
// MaxSizes. The difference is that finite values now have priority over Dynamic, so
// that min(3, Dynamic) gives 3. Indeed, whatever the actual value is (between 0 and 3),
...
#define EIGEN_SCALAR_BINARYOP_EXPR_RETURN_TYPE(SCALAR, EXPR, OPNAME)                   \
    CwiseBinaryOp<                                                                     \
        EIGEN_CAT(EIGEN_CAT(internal::scalar_, OPNAME), _op) < SCALAR,                 \
        typename internal::traits<EXPR>::Scalar>,                                      \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHERIGHT(METHOD, OPNAME)                         \
    ...
    {                                                                                  \
        typedef typename internal::promote_scalar_arg<                                 \
            Scalar,                                                                    \
            T,                                                                         \
            EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, Scalar, T)>::type PromotedT;         \
...
#define EIGEN_MAKE_SCALAR_BINARY_OP_ONTHELEFT(METHOD, OPNAME)                          \
    ...
    {                                                                                  \
        typedef typename internal::promote_scalar_arg<                                 \
            Scalar,                                                                    \
            T,                                                                         \
            EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, T, Scalar)>::type PromotedT;         \
...
#if (defined(_CPPUNWIND) || defined(__EXCEPTIONS)) && !defined(EIGEN_CUDA_ARCH)        \
    && !defined(EIGEN_EXCEPTIONS) && !defined(EIGEN_USE_SYCL)                          \
...
// The all function is used to enable a variadic version of eigen_assert which can take
// a parameter pack as its input.
namespace Eigen
{
namespace internal
{
