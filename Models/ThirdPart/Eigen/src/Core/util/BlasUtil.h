...
template <
    typename Scalar, typename Index, typename DataMapper, int nr, int StorageOrder,
    bool Conjugate = false, bool PanelMode = false>
struct gemm_pack_rhs;

template <
    typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
    typename Packet, int StorageOrder, bool Conjugate = false, bool PanelMode = false>
struct gemm_pack_lhs;

template <
    typename Index, typename LhsScalar, int LhsStorageOrder, bool ConjugateLhs,
    typename RhsScalar, int RhsStorageOrder, bool ConjugateRhs, int ResStorageOrder,
    int ResInnerStride>
struct general_matrix_matrix_product;

template <
    typename Index, typename LhsScalar, typename LhsMapper, int LhsStorageOrder,
    bool ConjugateLhs, typename RhsScalar, typename RhsMapper, bool ConjugateRhs,
...
public:
    ...
    BlasLinearMapper(Scalar *data, Index incr = 1) :
        m_data(data)
    {
...
template <
    typename Scalar, typename Index, int StorageOrder, int AlignmentType = Unaligned,
    int Incr = 1>
...
template <
    typename Index, typename Scalar, typename Packet, int n, int idx, int StorageOrder>
...
public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE
    blas_data_mapper(Scalar *data, Index stride, Index incr = 1) :
        m_data(data),
        m_stride(stride)
    {
        EIGEN_ONLY_USED_FOR_DEBUG(incr);
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
    storePacketBlock(Index i, Index j, const PacketBlock<SubPacket, n> &block) const
    {
...
template <
    typename Scalar, typename Index, int StorageOrder, int AlignmentType, int Incr>
class blas_data_mapper
{
public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE
    blas_data_mapper(Scalar *data, Index stride, Index incr) :
        m_data(data),
        m_stride(stride), m_incr(incr)
    {}
    ...
    {
        return m_data
            [StorageOrder == RowMajor ? j * m_incr.value() + i * m_stride :
    ...
    // storePacketBlock_helper defines a way to access values inside the PacketBlock,
    // this is essentially required by the Complex types.
    ...
    // This function stores a PacketBlock on m_data, this approach is really quite slow
    // compare to Incr=1 and should be avoided when possible.
    template <typename SubPacket, int n>
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
    storePacketBlock(Index i, Index j, const PacketBlock<SubPacket, n> &block) const
    {
...
struct blas_traits
{
    ...
    typedef typename conditional<
        bool(HasUsableDirectAccess), ExtractType,
...
struct blas_traits<CwiseBinaryOp<
    scalar_product_op<Scalar>, const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain>,
    NestedXpr>> : blas_traits<NestedXpr>
{
    enum
    {
...
struct blas_traits<CwiseBinaryOp<
    ...
    const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain>>> : blas_traits<NestedXpr>
{
    enum
    {
        HasScalarFactor = true
...
struct blas_traits<CwiseBinaryOp<
    scalar_product_op<Scalar>, const CwiseNullaryOp<scalar_constant_op<Scalar>, Plain1>,
...
struct blas_traits<Transpose<NestedXpr>> : blas_traits<NestedXpr>
{
    ...
    typedef Transpose<const typename Base::_ExtractType>
        ExtractType;  // const to get rid of a compile error; anyway blas traits are
                      // only used on the RHS
    typedef Transpose<const typename Base::_ExtractType> _ExtractType;
    typedef typename conditional<
        bool(Base::HasUsableDirectAccess), ExtractType,
...
template <
    typename T, bool HasUsableDirectAccess = blas_traits<T>::HasUsableDirectAccess>
...
struct combine_scalar_factors_impl
{
    ...
    run(const Lhs &lhs, const Rhs &rhs)
    {
