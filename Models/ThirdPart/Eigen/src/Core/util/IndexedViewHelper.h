 ...
 * reference the last element/row/columns of the underlying vector or matrix once passed
 * to DenseBase::operator()(const RowIndices&, const ColIndices&).
 *
...
// IndexedViewCompatibleType/makeIndexedViewCompatible turn an arbitrary object of type
// T into something usable by MatrixSlice The generic implementation is a no-op
...
// Turn a single index into something that looks like an array (i.e., that exposes a
// .size(), and operator[](int) methods)
template <typename T, int XprSize>
struct IndexedViewCompatibleType<
    T, XprSize, typename internal::enable_if<internal::is_integral<T>::value>::type>
{
...
EIGEN_DEPRECATED static const last_t last =
    Eigen::last;  // PLEASE use Eigen::last   instead of Eigen::placeholders::last
EIGEN_DEPRECATED static const end_t end =
    Eigen::lastp1;  // PLEASE use Eigen::lastp1 instead of Eigen::placeholders::end
}  // namespace placeholders
