...
inline void throw_std_bad_alloc()
{
    ...
    //
    // calls to "::operator new" are to be treated as opaque function calls (i.e no
    // inlining), and as a consequence the code in the #else block triggers the hipcc
    // warning : "no overloaded function has restriction specifiers that are compatible
    // with the ambient context"
    //
    // "throw_std_bad_alloc" has the EIGEN_DEVICE_FUNC attribute, so it seems that hipcc
    // expects the same on "operator new" Reverting code back to the old version in this
    // #if block for the hipcc compiler
    //
    new int[huge];
#else
    ...
    std::size_t size, std::size_t alignment = EIGEN_DEFAULT_ALIGN_BYTES)
{
    ...
    void *aligned = reinterpret_cast<void *>(
        (reinterpret_cast<std::size_t>(original) & ~(std::size_t(alignment - 1)))
        + alignment);
...
/** \internal Allocates \a size bytes. The returned pointer is guaranteed to have 16 or
 * 32 bytes alignment depending on the requirements. On allocation error, the returned
 * pointer is null, and std::bad_alloc is thrown.
 */
...
/** \internal Allocates \a size bytes. If Align is true, then the returned ptr is
 * 16-byte-aligned. On allocation error, the returned pointer is null, and a
...
/** \internal Allocates \a size objects of type T. The returned pointer is guaranteed to
 * have 16 bytes alignment. On allocation error, the returned pointer is undefined, but
 * a std::bad_alloc is thrown. The default constructor of T is called.
 */
template <typename T>
...
/** \internal Returns the index of the first element of the array that is well aligned
 * with respect to the requested \a Alignment.
 *
 * \tparam Alignment requested alignment in Bytes.
 * \param array the address of the start of the array
 * \param size the size of the array
 *
 * \note If no element of the array is well aligned or the requested alignment is not a
 * multiple of a scalar, the size of the array is returned. For example with SSE, the
 * requested alignment is typically 16-bytes. If packet size for the given scalar type
 * is 1, then everything is considered well-aligned.
 *
 * \note Otherwise, if the Alignment is larger that the scalar size, we rely on the
 * assumptions that sizeof(Scalar) is a power of 2. On the other hand, we do not assume
 * that the array address is a multiple of sizeof(Scalar), as that fails for example
 * with Scalar=double on certain 32-bit platforms, see bug #79.
 *
...
EIGEN_DEVICE_FUNC inline Index first_aligned(const Scalar *array, Index size)
{
    ...
    if (AlignmentSize <= 1)
    {
        // Either the requested alignment if smaller than a scalar, or it exactly match
        // a 1 scalar so that all elements of the array have the same alignment.
        return 0;
    }
    else if ((UIntPtr(array) & (sizeof(Scalar) - 1)) || (Alignment % ScalarSize) != 0)
    {
        // The array is not aligned to the size of a single scalar, or the requested
        // alignment is not a multiple of the scalar size. Consequently, no element of
        // the array is well aligned.
        return size;
    }
    else
    {
        Index first =
            (AlignmentSize - (Index((UIntPtr(array) / sizeof(Scalar))) & AlignmentMask))
            & AlignmentMask;
...
/** \internal Returns the index of the first element of the array that is well aligned
 * with respect the largest packet requirement. \sa first_aligned(Scalar*,Index) and
...
/** \internal Returns the smallest integer multiple of \a base and greater or equal to
 * \a size
 */
...
// use memcpy on trivial types, i.e., on types that does not require an initialization
// ctor.
...
{
    smart_memmove_helper<T, !NumTraits<T>::RequireInitialization>::run(
        start, end, target);
}
...
// This helper class construct the allocated memory, and takes care of destructing and
// freeing the handled data at destruction time. In practice this helper class is mainly
// useful to avoid memory leak in case of exceptions.
template <typename T>
...
public:
     ...
     * This constructor takes care of constructing/initializing the elements of the
     *buffer if required by the scalar type T (see NumTraits<T>::RequireInitialization).
     * In this case, the buffer elements will also be destructed when this handler will
     *be destructed. Finally, if \a dealloc is true, then the pointer \a ptr is freed.
     **/
    EIGEN_DEVICE_FUNC
...
template <
    typename Xpr, int NbEvaluations,
    bool                                            MapExternalBuffer =
        nested_eval<Xpr, NbEvaluations>::Evaluate &&Xpr::MaxSizeAtCompileTime
        == Dynamic>
...
struct local_nested_eval_wrapper<Xpr, NbEvaluations, true>
{
    ...
    local_nested_eval_wrapper(const Xpr &xpr, Scalar *ptr) :
        object(
            ptr == 0 ? reinterpret_cast<Scalar *>(
                Eigen::internal::aligned_malloc(sizeof(Scalar) * xpr.size())) :
                       ptr,
            xpr.rows(), xpr.cols()),
        m_deallocate(ptr == 0)
    {
...
/** \internal
 *
 * The macro ei_declare_aligned_stack_constructed_variable(TYPE,NAME,SIZE,BUFFER)
 * declares, allocates, and construct an aligned buffer named NAME of SIZE elements of
 * type TYPE on the stack if the size in bytes is smaller than
 * EIGEN_STACK_ALLOCATION_LIMIT, and if stack allocation is supported by the platform
 * (currently, this is Linux, OSX and Visual Studio only). Otherwise the memory is
 * allocated on the heap. The allocated buffer is automatically deleted when exiting the
 * scope of this declaration. If BUFFER is non null, then the declared variable is
 * simply an alias for BUFFER, and no allocation/deletion occurs. Here is an example:
 * \code
 * {
 ...
 * \endcode
 * with the advantage of using aligned stack allocation even if the maximal size of XPR
 * at compile time is unknown. This is accomplished through alloca if this later is
 * supported and if the required number of bytes is below EIGEN_STACK_ALLOCATION_LIMIT.
 */
#ifdef EIGEN_ALLOCA

#if EIGEN_DEFAULT_ALIGN_BYTES > 0
   // We always manually re-align the result of EIGEN_ALLOCA.
// If alloca is already aligned, the compiler should be smart enough to optimize away
// the re-alignment.
...
#define ei_declare_local_nested_eval(XPR_T, XPR, N, NAME)                              \
    Eigen::internal::local_nested_eval_wrapper<XPR_T, N> EIGEN_CAT(NAME, _wrapper)(    \
        ...
        reinterpret_cast<typename XPR_T::Scalar *>(                                    \
            ((Eigen::internal::local_nested_eval_wrapper<XPR_T, N>::                   \
                  NeedExternalBuffer)                                                  \
             && ((sizeof(typename XPR_T::Scalar) * XPR.size())                         \
                ...
                0));                                                                   \
    typename Eigen::internal::local_nested_eval_wrapper<XPR_T, N>::ObjectType NAME(    \
        EIGEN_CAT(NAME, _wrapper).object)
 ...
 * By default, it will thus provide at least 16 bytes alignment and more in following
 * cases:
...
// Case for x64 with PIC. In theory this is only a problem with recent gcc and with
// medium or large code model, not with the default small code model. However, we cannot
// detect which code model is used, and the xchg overhead is negligible anyway.
#define EIGEN_CPUID(abcd, func, id)                                                    \
...
/** \internal
 * Queries and returns the cache sizes in Bytes of the L1, L2, and L3 data caches
 * respectively */
