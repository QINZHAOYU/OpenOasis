...
{
    return IndexType(
        ((n_col) + 1) * sizeof(ColStructure<IndexType>) / sizeof(IndexType));
}
...
{
    return IndexType(
        ((n_row) + 1) * sizeof(RowStructure<IndexType>) / sizeof(IndexType));
}

// Prototypes of non-user callable routines
template <typename IndexType>
static IndexType init_rows_cols(
    IndexType n_row, IndexType n_col, RowStructure<IndexType> Row[],
...
static IndexType find_ordering(
    IndexType n_row, IndexType n_col, IndexType Alen, RowStructure<IndexType> Row[],
...
static void detect_super_cols(
    ColStructure<IndexType> Col[], IndexType A[], IndexType head[], IndexType row_start,
...
/**
 * \brief  Computes a column ordering using the column approximate minimum degree
 * ordering
 *
...
static bool compute_ordering(
    IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p,
    double knobs[NKnobs], IndexType stats[NStats])
{
    ...
    Colamd::init_scoring(
        n_row, n_col, Row, Col, A, p, knobs, &n_row2, &n_col2, &max_deg);

    /* === Order the supercolumns =========================================== */

    ngarbage = Colamd::find_ordering(
        n_row, n_col, Alen, Row, Col, A, p, n_col2, max_deg, 2 * nnz);
...
)
{
    ...
    dense_row_count = numext::maxi(
        IndexType(0), numext::mini(IndexType(knobs[Colamd::DenseRow] * n_col), n_col));
    dense_col_count = numext::maxi(
        IndexType(0), numext::mini(IndexType(knobs[Colamd::DenseCol] * n_row), n_row));
