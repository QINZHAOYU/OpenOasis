...
template <typename StorageIndex>
StorageIndex cs_tdfs(
    StorageIndex j, StorageIndex k, StorageIndex *head, const StorageIndex *next,
 ...
 *
 * Note that the input matrix \a C must be complete, that is both the upper and lower
 * parts have to be stored, as well as the diagonal entries. On exit the values of C are
 * destroyed */
    ...
    PermutationMatrix<Dynamic, Dynamic, StorageIndex> &perm)
{
    ...
    StorageIndex d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln,
                              dense, nzmax, mindeg = 0, nvi, nvj, nvk, mark, wnvi, ok,
                              nel = 0, p, p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, t, h;
    ...
    while (nel < n) /* while (selecting pivots) do */
    {
        ...
        mark = internal::cs_wclear<StorageIndex>(
            mark, lemax, w, n);        /* clear w if necessary */
        ...
        mark =
            internal::cs_wclear<StorageIndex>(mark + lemax, lemax, w, n); /* clear w */
    ...
    for (k = 0, i = 0; i <= n; i++) /* postorder the assembly tree */
    {
        if (Cp[i] == -1)
            k = internal::cs_tdfs<StorageIndex>(
                i, k, head, next, perm.indices().data(), w);
    }
