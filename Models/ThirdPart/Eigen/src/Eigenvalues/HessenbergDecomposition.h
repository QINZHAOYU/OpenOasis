...
public:
     ...
     *
     * \param [in] size  The size of the matrix whose Hessenberg decomposition will be
     * computed.
     *
     ...
     *  - the rest of the lower part contains the Householder vectors that, combined
     *with Householder coefficients returned by householderCoefficients(), allows to
     *reconstruct the matrix Q as \f$ Q = H_{N-1} \ldots H_1 H_0 \f$. Here, the matrices
     *\f$ H_i \f$ are the Householder transformations \f$ H_i = (I - h_i v_i v_i^T) \f$
 ...
 */
template <typename MatrixType>
void HessenbergDecomposition<MatrixType>::_compute(
    ...
    for (Index i = 0; i < n - 1; ++i)
    {
        ...
        matA.bottomRightCorner(remainingSize, remainingSize)
            .applyHouseholderOnTheLeft(
                matA.col(i).tail(remainingSize - 1), h, &temp.coeffRef(0));

        // A = A H'
        matA.rightCols(remainingSize)
            .applyHouseholderOnTheRight(
                matA.col(i).tail(remainingSize - 1),
                numext::conj(h),
                &temp.coeffRef(0));
    }
}
