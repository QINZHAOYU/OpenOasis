...
template <
    typename Lhs, typename Rhs, int Mode,
        ...
        for (Index col = 0; col < other.cols(); ++col)
        {
            for (Index i = lhs.cols() - 1; i >= 0; --i)
            {
                ...
                if (tmp != Scalar(0))  // optimization when other is actually sparse
                {
                    if (!(Mode & UnitDiag))
                    {
                        // TODO replace this by a binary search. make sure the binary
                        // search is safe for partially sorted elements
                        LhsIterator it(lhsEval, i);
                        while (it && it.index() != i)
                            ++it;
                        eigen_assert(it && it.index() == i);
...
{
    eigen_assert(
        derived().cols() == derived().rows() && derived().cols() == other.rows());
    ...
    SparseMatrixBase<OtherDerived> &other) const
{
    ...
    internal::sparse_solve_triangular_sparse_selector<
        ExpressionType,
        OtherDerived,
        Mode>::run(derived().nestedExpression(), other.derived());
