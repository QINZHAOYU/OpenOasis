...
public:
    ...
    void resize(Index size, double reserveSizeFactor = 0)
    {
        if (m_allocatedSize < size)
        {
            Index realloc_size =
                (std::min<
                    Index>)(NumTraits<StorageIndex>::highest(), size + Index(reserveSizeFactor * double(size)));
     ...
     * If the value does not exist, then the value \a defaultValue is returned without
     * any insertion. */
    ...
    inline Scalar atInRange(
        Index start, Index end, Index key, const Scalar &defaultValue = Scalar(0)) const
    {
        if (start >= end)
            return defaultValue;
    ...
    inline Scalar &atWithInsertion(Index key, const Scalar &defaultValue = Scalar(0))
    {
        ...
        if (id >= m_size || m_indices[id] != key)
        {
            if (m_allocatedSize < m_size + 1)
            {
                ...
                {
                    internal::smart_copy(
                        m_values + id, m_values + m_size, newValues.ptr() + id + 1);
                    internal::smart_copy(
                        m_indices + id, m_indices + m_size, newIndices.ptr() + id + 1);
                }
            ...
            else if (m_size > id)
            {
                internal::smart_memmove(
                    m_values + id, m_values + m_size, m_values + id + 1);
                internal::smart_memmove(
                    m_indices + id, m_indices + m_size, m_indices + id + 1);
            }
            m_size++;
    ...
    void moveChunk(Index from, Index to, Index chunkSize)
    {
        ...
        if (to > from && from + chunkSize > to)
        {
            // move backward
            internal::smart_memmove(
                m_values + from, m_values + from + chunkSize, m_values + to);
            internal::smart_memmove(
                m_indices + from, m_indices + from + chunkSize, m_indices + to);
        }
        else
        {
            internal::smart_copy(
