...
{
    // by default sparse evaluation do not alias, so we can safely bypass the generic
    // call_assignment routine
    internal::Assignment<
        Derived,
        OtherDerived,
...
void assign_sparse_to_sparse(DstXprType &dst, const SrcXprType &src)
{
    ...
    if ((!transpose) && src.isRValue())
    {
        ...
        dst.setZero();
        dst.reserve((std::min)(
            src.rows() * src.cols(), (std::max)(src.rows(), src.cols()) * 2));
    ...
    else
    {
        ...
        eigen_assert(
            (((internal::traits<DstXprType>::SupportedAccessPatterns
        ...
        for (Index j = 0; j < outerEvaluationSize; ++j)
        {
            ...
            for (typename SrcEvaluatorType::InnerIterator it(srcEvaluator, j); it; ++it)
            {
                ...
                temp.insertBackByOuterInner(
                    Flip ? it.index() : j, Flip ? j : it.index()) = v;
            }
        }
        temp.finalize();
    ...
    static void run(DstXprType &dst, const SrcXprType &src, const Functor &func)
    {
        ...
        const Index outerEvaluationSize =
            (internal::evaluator<SrcXprType>::Flags & RowMajorBit) ? src.rows() :
                                                                     src.cols();
        for (Index j = 0; j < outerEvaluationSize; ++j)
            for (typename internal::evaluator<SrcXprType>::InnerIterator i(srcEval, j);
                 i;
                 ++i)
...
struct assignment_from_dense_op_sparse
{
    ...
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        typename internal::enable_if<internal::is_same<
            typename internal::evaluator_traits<Rhs>::Shape, DenseShape>::value>::type
        run(DstXprType &dst,
            const CwiseBinaryOp<
                internal::scalar_sum_op<Scalar, Scalar>, const Lhs, const Rhs> &src,
    ...
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
        ...
        run(DstXprType &dst,
            const CwiseBinaryOp<
                internal::scalar_difference_op<Scalar, Scalar>, const Lhs, const Rhs>
                &src,
            const internal::assign_op<typename DstXprType::Scalar, Scalar> & /*func*/)
    {
...
#define EIGEN_CATCH_ASSIGN_DENSE_OP_SPARSE(ASSIGN_OP, BINOP, ASSIGN_OP2)               \
    ...
    struct Assignment<                                                                 \
        ...
        typename internal::enable_if<                                                  \
            internal::is_same<                                                         \
                typename internal::evaluator_traits<Lhs>::Shape,                       \
                DenseShape>::value                                                     \
            || internal::is_same<                                                      \
                typename internal::evaluator_traits<Rhs>::Shape,                       \
...
// NOTE we need to specialize it for Sparse2Sparse to avoid ambiguous specialization
// error
template <typename DstXprType, typename DecType, typename RhsType, typename Scalar>
struct Assignment<
    DstXprType, Solve<DecType, RhsType>, internal::assign_op<Scalar, Scalar>,
    Sparse2Sparse>
{
...
struct Assignment<DstXprType, SrcXprType, Functor, Diagonal2Sparse>
{
    ...
    static void
    run(SparseMatrix<Scalar, Options, StorageIndex> &dst, const SrcXprType &src,
