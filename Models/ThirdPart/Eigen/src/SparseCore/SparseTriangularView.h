 ...
 * additional methods which are available for sparse expressions only.
 *
...
public:
    ...
    typedef
        typename internal::remove_all<MatrixTypeNested>::type MatrixTypeNestedCleaned;
...
public:
    ...
    public:
            ...
            m_containsDiag(Base::outer() < xprEval.m_arg.innerSize())
        {
            ...
            {
                while ((*this)
                       && ((HasUnitDiag || SkipDiag) ? this->index() <= outer :
                                                       this->index() < outer))
                    Base::operator++();
                if (HasUnitDiag)
