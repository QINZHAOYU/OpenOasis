...
protected:
    ...
    {
        // if the size of the matrix is not too large, let's allocate a bit more than
        // needed such that we can handle dense vector even in sparse mode.
        delete[] m_buffer;
        if (size < 1000)
        {
            Index allocSize =
...
{
    m_mode = mode;
    // This is only necessary in sparse mode, but we set these unconditionally to avoid
    // some maybe-uninitialized warnings if (m_mode==IsSparse)
    {
        m_llSize  = 0;
        m_llStart = -1;
    }
