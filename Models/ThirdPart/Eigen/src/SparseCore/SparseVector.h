 ...
 *
 * See http://www.netlib.org/linalg/html_templates/node91.html for details on the
 * storage scheme.
 *
...
public:
    ...
    {
        eigen_assert(
            IsColVector ? (col == 0 && row >= 0 && row < m_size) :
...
public:
    ...
    {
        eigen_assert(
            (IsColVector ? cols : rows) == 1 && "Outer dimension must equal 1");
     ...
     *
     * If the size of the vector is decreased, then the storage of the out-of bounds
     * coefficients is kept and reserved. Call .data().squeeze() to free extra memory.
     *
     * \sa reserve(), setZero()
     */
     ...
     * Overloaded for performance: this version performs a \em shallow swap by swapping
     * pointers and attributes only. \sa SparseMatrixBase::swap()
     */
...
public:
    ...
    // These two functions were here in the 3.1 release, so let's keep them in case some
    // code rely on them.
...
protected:
    static void check_template_parameters()
    {
        ...
        EIGEN_STATIC_ASSERT(
            (_Options & (ColMajor | RowMajor)) == Options,
