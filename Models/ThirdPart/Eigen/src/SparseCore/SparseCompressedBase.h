 ...
 *
 * This class defines the common interface for all derived classes implementing the
 * compressed sparse storage format, such as:
 *  - SparseMatrix
...
public:
     ...
     *
     * \warning this method is for \b compressed \b storage \b only, and it will trigger
...
protected:
    ...
    /** \internal return the index of the coeff at (row,col) or just before if it does
     * not exist. This is an analogue of std::lower_bound.
     */
    internal::LowerBoundIndex lower_bound(Index row, Index col) const
    {
        ...
        Index end   = this->isCompressed() ?
                          this->outerIndexPtr()[outer + 1] :
        ...
        p.value = std::lower_bound(
                      this->innerIndexPtr() + start, this->innerIndexPtr() + end, inner)
                  - this->innerIndexPtr();
...
public:
    ...
    InnerIterator(const InnerIterator &other) :
        m_values(other.m_values), m_indices(other.m_indices), m_outer(other.m_outer),
        m_id(other.m_id), m_end(other.m_end)
    {}
    ...
    explicit InnerIterator(const SparseCompressedBase &mat) :
        m_values(mat.valuePtr()), m_indices(mat.innerIndexPtr()), m_outer(0), m_id(0),
        m_end(mat.nonZeros())
    {
    ...
    explicit InnerIterator(
        ...
        m_indices(data.indexPtr()), m_outer(0), m_id(0), m_end(data.size())
    {
        EIGEN_STATIC_ASSERT_VECTOR_ONLY(Derived);
    }
...
public:
    ...
    explicit ReverseInnerIterator(
        const internal::CompressedStorage<Scalar, StorageIndex> &data) :
        m_values(data.valuePtr()),
        m_indices(data.indexPtr()), m_outer(0), m_start(0), m_id(data.size())
    {
        EIGEN_STATIC_ASSERT_VECTOR_ONLY(Derived);
    }
