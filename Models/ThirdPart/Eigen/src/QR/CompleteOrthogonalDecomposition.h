...
struct traits<CompleteOrthogonalDecomposition<_MatrixType>> : traits<_MatrixType>
{
 ...
 * \mathbf{Z} \f] by using Householder transformations. Here, \b P is a permutation
 * matrix, \b Q and \b Z are unitary matrices and \b T an upper triangular matrix of
...
public:
    ...
    explicit CompleteOrthogonalDecomposition(const EigenBase<InputType> &matrix) :
        ...
        m_zCoeffs((std::min)(matrix.rows(), matrix.cols())), m_temp(matrix.cols())
    {
    ...
    explicit CompleteOrthogonalDecomposition(EigenBase<InputType> &matrix) :
        m_cpqr(matrix.derived()), m_zCoeffs((std::min)(matrix.rows(), matrix.cols())),
        m_temp(matrix.cols())
    {
        computeInPlace();
    }
            ...
            if (k > 0)
            {
                ...
                m_cpqr.m_qr.topRightCorner(k, cols - rank + 1)
                    .applyHouseholderOnTheRight(
                        m_cpqr.m_qr.row(k).tail(cols - rank).adjoint(),
                        m_zCoeffs(k),
                        &m_temp(0));
            }
            if (k != rank - 1)
    ...
    Rhs &rhs) const
{
    ...
    for (Index k = 0; k < rank; ++k)
    {
        ...
        rhs.middleRows(rank - 1, cols - rank + 1)
            .applyHouseholderOnTheLeft(
                matrixQTZ().row(k).tail(cols - rank).adjoint(), zCoeffs()(k), &temp(0));
        if (k != rank - 1)
        {
            rhs.row(k).swap(rhs.row(rank - 1));
        }
